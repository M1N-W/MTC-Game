/**
 * 🤖 MTC: ENHANCED EDITION - AI System (REFACTORED)
...
const AI_CONFIG = {
    ...
    model: 'gemini-1.5-flash',  // โมเดลที่ใช้ (เปลี่ยนได้ง่าย)
    
    // โมเดลที่แนะนำ:
    // - 'gemini-1.5-flash' (เร็ว, ฟรี) ⭐ แนะนำ
    // - 'gemini-1.5-pro' (ดี, ช้ากว่า)
    // - 'gemini-2.0-flash-exp' (ทดลอง, อาจจะไม่มี)
...
class GeminiAI {
    ...
    async getBossTaunt(situation) {
        ...
        // ลองใช้ AI
        if (this.enabled) {
            const prompt = `คุณคือ "ครูมานพ" ครูคณิตศาสตร์ที่เข้มงวด สถานการณ์: ${situation}
ตอบ: 1 ประโยคสั้นๆ ภาษาไทยพูดจาเหมือนครู (ไม่เกิน 15 คำ) ห้ามมี emoji ห้ามใช้เครื่องหมาย * หรือ ** ตอบเป็นข้อความธรรมดา`;
...
คะแนน: ${score}, Wave: ${wave}
ตอบ: ความเห็นครู 1 ประโยคสั้นๆ ภาษาไทย (ไม่เกิน 20 คำ) ห้ามมี emoji`;
    ...
    async getMissionName() {
        ...
        // ลองใช้ AI
        if (this.enabled) {
            const prompt = `สร้างชื่อภารกิจ/ภาระกิจเกี่ยวกับคณิตศาสตร์ ภาษาไทย สั้นๆ (3-5 คำ) ห้ามมี emoji`;
            const result = await this.generate(prompt);
            if (result) {
