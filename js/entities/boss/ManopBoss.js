...
class KruManop extends BossBase {
  ...
  constructor(difficulty = 1, enablePhase2 = false, enablePhase3 = false) {
    ...
    this._phase2Threshold =
      p2t && p2t[enc] != null ? p2t[enc] : BALANCE.boss.phase2Threshold;
    this._phase3Threshold =
      p3t && p3t[enc] != null ? p3t[enc] : BALANCE.boss.phase3Threshold ?? 0.25;
    ...
    this.skills = {
      ...
      redPen: { cd: 0, max: 8.0 }, // New: Red Pen Correction cooldown
      zeroScore: { cd: 0, max: 10.0 }, // New: Zero Score Judgment cooldown
    };
  ...
  update(dt, player) {
    ...
    for (let s in this.skills)
      if (this.skills[s].cd > 0) this.skills[s].cd -= dt;
    ...
    // ── Phase 2 transition ───────────────────────────────
    if (
      this.hp < this.maxHp * this._phase2Threshold &&
      this.phase === 1 &&
      this.enablePhase2
    ) {
      this.phase = 2;
      this.isEnraged = true;
      this.moveSpeed =
        BALANCE.boss.moveSpeed * BALANCE.boss.phase2.enrageSpeedMult;

      // ── PHASE 2 FEEL IMPLEMENTATION ──────────────────
      // 1. Visual Feedback
      spawnFloatingText(
        "PHASE 2: THE FINAL EXAM",
        this.x,
        this.y - 80,
        "#ef4444",
        40
      ...
      addScreenShake(30);
      spawnParticles(this.x, this.y, 40, "#ef4444");
      spawnParticles(this.x, this.y, 25, "#ffffff"); // Chalk dust

      // 2. Dialogue (Storyboard Panel 2)
      if (window.UIManager) {
        window.UIManager.showVoiceBubble(
          "คะแนนสะสม: ศูนย์ · เวลาที่เหลือ: ไม่มี",
          this.x,
          this.y - 50
        );
        setTimeout(() => {
          if (this.dead) return;
          window.UIManager.showVoiceBubble(
            "ความน่าจะเป็นที่จะรอด... คำนวณเองแล้วกัน",
            this.x,
            this.y - 50
          );
        }, 1800);
      ...
      // 3. Audio Transition (Storyboard Panel 3)
      Audio.playBossSpecial();
      // Assuming 'boss_phase2' is available in audio.js/config.js
      // If not yet defined, we'll use boss BGM but with a special cue
      if (typeof Audio.playBGM === "function") {
        Audio.playBGM("boss"); // Restarting or shifting track if supported
      }
      ...
      this._summonDog();
      // Fresh phase 2 speech
      this.sayTimer = -5.0; // delay next random taunt
    }

    // ── Phase 3 transition — "The Goldfish Lover" ────────
    if (
      this.hp < this.maxHp * this._phase3Threshold &&
      this.phase === 2 &&
      this.enablePhase3
    ) {
      this.phase = 3;
      this.moveSpeed =
        BALANCE.boss.moveSpeed * BALANCE.boss.phase2.enrageSpeedMult * 1.15;
    ...
    if (this._dogPackActive) {
      ...
      if (this._dogPackTimer >= 0.5 && !this._dogPackFired) {
        ...
        if (this.dog && !this.dog.dead) {
          const ddx = player.x - this.dog.x,
            ddy = player.y - this.dog.y;
        ...
        for (let i = 0; i < bullets; i++) {
          ...
          projectileManager.add(
            new Projectile(
              ...
              this.y,
              a,
              BALANCE.boss.ultimateProjectileSpeed,
              BALANCE.boss.ultimateDamage * 0.85,
              "#ef4444",
              true,
              "enemy"
            )
        ...
        if (typeof window.specialEffects !== "undefined") {
          window.specialEffects.push(
            new ExpandingRing(this.x, this.y, "#d97706", 260, 0.5)
          );
        }
        addScreenShake(18);
        Audio.playBossSpecial();
    ...
    if (this.log457State === "charging") {
      ...
      this.hp = Math.min(
        this.maxHp,
        this.hp + this.maxHp * BALANCE.boss.log457HealRate * dt
      );
      ...
      if (this.log457Timer >= BALANCE.boss.log457ChargeDuration) {
        ...
        if (typeof window.specialEffects !== "undefined") {
          window.specialEffects.push(
            new ExpandingRing(this.x, this.y, "#facc15", 220, 0.5)
          );
        }
        this.speak("0.6767!");
      }
    ...
    if (this.state === "CHASE") {
      ...
      if (this.timer > 2) {
        ...
        if (typeof playerAnalyzer !== "undefined") {
          ...
          if (pattern === "kiting" && playerAnalyzer.kitingScore() > 0.55) {
            if (this.skills.graph.cd <= 0) {
              this.useDeadlyGraph(player);
              return;
            }
            if (this.phase >= 2 && this.skills.chalkWall.cd <= 0) {
          ...
          if (pattern === "standing" && playerAnalyzer.standingScore() > 0.6) {
            if (this.skills.slam.cd <= 0) {
              this.useEquationSlam();
              return;
            }
            if (this.skills.log.cd <= 0) {
              this.useLog457();
              return;
            }
          }
        ...
        const barkChance = this.phase === 2 ? 0.4 : 0;
        // DogPackCombo — highest phase-2 priority: dog must be alive
        if (
          this.phase >= 2 &&
          this.dog &&
          !this.dog.dead &&
          this.skills.dogPack.cd <= 0 &&
          Math.random() < 0.3
        ) {
          this.useDogPackCombo(player);
        } else if (
          this.phase >= 2 &&
          this.skills.redPen.cd <= 0 &&
          Math.random() < 0.4
        ) {
          this.useRedPenCorrection(player);
        } else if (
          this.phase >= 2 &&
          this.skills.zeroScore.cd <= 0 &&
          Math.random() < 0.35
        ) {
          this.useZeroScoreJudgment(player);
        } else if (this.skills.log.cd <= 0 && Math.random() < 0.2) {
          this.useLog457();
        } else if (this.skills.graph.cd <= 0 && Math.random() < 0.25) {
          this.useDeadlyGraph(player);
        } else if (
          this.phase === 2 &&
          this.skills.bark.cd <= 0 &&
          Math.random() < barkChance
        ) {
          this.bark(player);
        } else if (
          this.phase >= 2 &&
          this.skills.chalkWall.cd <= 0 &&
          Math.random() < 0.35
        ) {
          this.useChalkWall(player);
        } else if (this.skills.slam.cd <= 0 && Math.random() < 0.3) {
          this.useEquationSlam();
        } else if (
          this.phase >= 2 &&
          this.skills.matrixGrid.cd <= 0 &&
          Math.random() < 0.35
        ) {
      ...
      if (this.timer > bf) {
        ...
        projectileManager.add(
          new Projectile(
            ...
            this.y,
            _aimAngle,
            BALANCE.boss.chalkProjectileSpeed,
            BALANCE.boss.chalkDamage,
            "#fff",
            false,
            "enemy"
          )
      ...
      if (!this._ultWound && this.timer > 0.65) {
        ...
        const bullets =
          this.phase === 2
            ? BALANCE.boss.phase2UltimateBullets
            : BALANCE.boss.ultimateBullets;
          ...
          for (let i = 0; i < bullets; i++) {
            ...
            projectileManager.add(
              new Projectile(
                ...
                this.y,
                a,
                BALANCE.boss.ultimateProjectileSpeed * 0.78,
                BALANCE.boss.ultimateDamage * 0.75,
                "#f97316",
                true,
                "enemy"
              )
        ...
        if (typeof window.specialEffects !== "undefined") {
          window.specialEffects.push(
            new ExpandingRing(this.x, this.y, "#facc15", 280, 0.65)
          );
          window.specialEffects.push(
            new ExpandingRing(this.x, this.y, "#ef4444", 200, 0.45)
          );
        }
        addScreenShake(22);
    ...
    if (d < this.radius + player.radius)
      player.takeDamage(
        BALANCE.boss.contactDamage * dt * (1 + this.log457AttackBonus)
      );
  ...
  bark(player) {
    ...
    window.specialEffects.push(
      new BarkWave(this.x, this.y, barkAngle, coneHalf, P2.barkRange)
    );
    const dx = player.x - this.x,
      dy = player.y - this.y,
      d = Math.hypot(dx, dy);
  ...
  useChalkWall(player) {
    ...
    const t = Math.min(0.6, 200 / Math.max(d, 1));
    const wx = this.x + (player.x - this.x) * t;
    const wy = this.y + (player.y - this.y) * t;

    if (
      typeof ChalkWall !== "undefined" &&
      typeof window.specialEffects !== "undefined"
    ) {
  ...
  // ── Phase 2: Red Pen Correction ───────────────────────────
  // Sweeps horizontal red lines across the arena.
  // Logic: Spawns a series of horizontal beam indicators, then fires.
  useRedPenCorrection(player) {
    this.skills.redPen.cd = this.skills.redPen.max;
    this.state = "CHASE";

    spawnFloatingText(
      "🖍️ RED PEN CORRECTION!",
      this.x,
      this.y - 80,
      "#ef4444",
      32
    );
    spawnFloatingText("FAIL! FAIL! FAIL!", this.x, this.y - 115, "#b91c1c", 20);

    // Use special effect to handle the multi-line warning and damage
    if (typeof window.specialEffects !== "undefined") {
      const count = 3;
      const spacing = 180;
      const startY = player.y - ((count - 1) * spacing) / 2;

      for (let i = 0; i < count; i++) {
        const wy = startY + i * spacing;
        // We'll use a modified DeadlyGraph or a new custom effect if available.
        // For now, let's spawn 3 DeadlyGraphs with zero X-offset to act as horizontal lines.
        // Note: DeadlyGraph(x1, y1, x2, y2, duration)
        ...
        window.specialEffects.push(
          new DeadlyGraph(
            ...
            this.x + arenaWidth / 2,
            wy,
            1.5 // shorter duration for high pressure
          )
        );
      }
    }
    ...
    addScreenShake(15);
    this.speak("Correcting your mistakes!");
    Audio.playBossSpecial();
  }

  // ── Phase 2: Zero Score Judgment ──────────────────────────
  // Spawns a countdown circle at the player's position.
  useZeroScoreJudgment(player) {
    this.skills.zeroScore.cd = this.skills.zeroScore.max;
    this.state = "CHASE";

    spawnFloatingText("💯 ZERO SCORE!", this.x, this.y - 80, "#ef4444", 32);
    spawnFloatingText(
      "3... 2... 1...",
      player.x,
      player.y - 110,
    ...
    // Use existing shockwave ring but with higher delay and custom visual
    if (typeof window.specialEffects !== "undefined") {
      // Create a custom countdown effect using ExpandingRing or similar
      // For now, let's trigger a delayed explosion at player's location
      const px = player.x,
        py = player.y;
      setTimeout(() => {
        if (this.dead) return;
        window.specialEffects.push(
          new ExpandingRing(px, py, "#ef4444", 220, 0.6)
        );
        // Damage player if they are still within the radius
        const d = dist(px, py, player.x, player.y);
        if (d < 220) {
          player.takeDamage(45); // High damage
          addScreenShake(20);
        }
      }, 1200); // 1.2s countdown
    }

    this.speak("You get ZERO!");
    Audio.playBossSpecial();
  }
  ...
  takeDamage(amt) {
    ...
    if (this.hp <= 0 && !this._waveSpawnLocked) {
      ...
      if (typeof Achievements !== "undefined") {
        Achievements.stats.manopKills =
          (Achievements.stats.manopKills ?? 0) + 1;
