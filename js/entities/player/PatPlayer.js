...
class PatPlayer extends Player {
    ...
    checkPassiveUnlock() {
        ...
        spawnFloatingText('⚔️ MELEE DAMAGE +15%', this.x, this.y - 105, '#fbbf24', 16);
        ...
        } catch (e) { console.warn('[MTC Save] PatPlayer passive:', e); }
    }
    ...
    update(dt, keys, mouse) {
        ...
        for (let i = 0; i < ghostCount; i++) {
            ...
            if (g.alpha <= 0) { g.alpha = 0; g.active = false; }
        }
        ...
        if (checkInput('r') && this.skills.iaido.cd <= 0 && this._iaidoPhase === 'none') {
    ...
    _beginIaidoCharge(S, mouse) {
        ...
        if (typeof Audio !== 'undefined' && Audio.playIaidoCharge) Audio.playIaidoCharge();
    }
    ...
    _executeIaidoFlash(S) {
        ...
        this._iaidoFlashLine = {
            x1: this._iaidoOriginX, y1: this._iaidoOriginY,
        ...
        if (hit) {
            ...
            if (typeof Audio !== 'undefined' && Audio.playIaidoStrike) Audio.playIaidoStrike();
        } else {
            // Miss — end immediately
            this._iaidoPhase = 'none';
            this._iaidoTimer = 0;
            this.skills.iaido.cd = (this.skills.iaido.max * this.skillCooldownMult) * 0.5; // miss = half CD
    ...
    _resolveIaidoDamage(S) {
        ...
        const critChance = (this.baseCritChance ?? 0.08) + (this._zanzoAmbushTimer > 0 ? (S.zanzoCritBonus ?? 0.40) : 0);
        ...
        spawnFloatingText(
            ...
            e.x, e.y - 40,
            isCrit ? '#facc15' : '#ef4444', isCrit ? 26 : 20
        );
        ...
        if (typeof Audio !== 'undefined' && Audio.playIaidoSheathe) Audio.playIaidoSheathe();
    ...
    _doSlashWave(S, mouse) {
        ...
        const extraCrit = this._zanzoAmbushTimer > 0 ? (S.zanzoCritBonus ?? 0.40) : 0;
        ...
        this.cooldowns.shoot = (wep.cooldown ?? 0.38) * this.cooldownMultiplier * this.skillCooldownMult;

        spawnParticles(sx, sy, 3, color);
        if (typeof Audio !== 'undefined' && Audio.playShoot) Audio.playShoot('katana');
    ...
    _doMeleeCombo(S, mouse) {
        const aimAngle = Math.atan2(mouse.wy - this.y, mouse.wx - this.x);
        const meleeRange = S.meleeRange ?? 150;
        const baseDmg = (S.weapons.katana.damage * (S.meleeDamageMulti ?? 1.8)) * this.damageMultiplier
        ...
        if (window.boss && !window.boss.dead) {
            if (Math.hypot(window.boss.x - this.x, window.boss.y - this.y) <= meleeRange + (window.boss.radius ?? 50)) {
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            let dmg = baseDmg;
            const isCrit = Math.random() < critChance;
            if (isCrit) { dmg *= (this.stats.critMultiplier ?? 2.4); this.goldenAuraTimer = 0.5; }
            ...
            spawnFloatingText(
                ...
                e.x, e.y - 30,
                isCrit ? '#facc15' : '#7ec8e3', isCrit ? 20 : 15
            );
        ...
        if (typeof Audio !== 'undefined' && Audio.playMeleeHit) Audio.playMeleeHit();
    }
    ...
    updateUI() {
        ...
        UIManager._setCooldownVisual('q-icon',
            Math.max(0, this.skills.zanzo.cd), this.skills.zanzo.max);

        // R — Iaido Strike
        UIManager._setCooldownVisual('r-icon',
            Math.max(0, this.skills.iaido.cd), this.skills.iaido.max);

        // R-Click — Blade Guard (show cooldown when not held)
        UIManager._setCooldownVisual('stealth-icon',
            Math.max(0, this._bladeGuardCooldown), S.bladeGuardCooldown ?? 5.0);

        // Dash
        UIManager._setCooldownVisual('dash-icon',
            Math.max(0, this.cooldowns?.dash ?? 0), S.dashCooldown ?? 1.6);
