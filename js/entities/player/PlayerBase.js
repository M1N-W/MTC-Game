...
class Player extends Entity {
  constructor(charId = "kao") {
    ...
    this._anim = {
      ...
      t: 0, // เวลาใน state ปัจจุบัน (วินาที)
      shootT: 0, // 0→1 decay — arm raise หลังยิง
      hurtT: 0, // 0→1 decay — flinch หลังโดนตี
      dashT: 0, // 0→1 decay — lean หลัง dash
      skillT: 0, // per-character skill cast timer
      // ── Phase 4 lerp fields ─────────────────────────────
      smoothMoveT: 0, // lerp of moveT — ป้องกัน snap บน/ออก
      smoothAngle: null, // lerp of entity.angle — เริ่ม null → init on first tick
    };
      ...
      for (let i = 0; i < unlocked.length; i++) {
        ...
        switch (r.type) {
          ...
          case "cdr":
            ...
            this.skillCooldownMult = Math.max(
              0.1,
              this.skillCooldownMult - r.value,
            );
            break;
        }
      }
  ...
  update(dt, keys, mouse) {
    ...
    if (this.groundedTimer > 0) {
      ...
      if (
        Math.floor((this.groundedTimer + dt) * 4) !==
        Math.floor(this.groundedTimer * 4)
      ) {
        spawnFloatingText(
          "⚡ DASH LOCKED!",
          this.x,
    ...
    let speedMult =
      (this.isInvisible ? S.stealthSpeedBonus : 1) *
      this.speedBoost *
      this.metaSpeedMult;
    if (this.passiveUnlocked && this.passiveSpeedBonus > 0)
      speedMult = Math.max(speedMult, this.passiveSpeedBonus);
    if ((this._speedWaveTimer ?? 0) > 0)
      speedMult *= this._speedWaveMult ?? 1.3;
    ...
    if (this.bladeGuardActive)
      speedMult *= this.stats?.bladeGuardSpeedMult ?? 0.6;
    // ── Second Wind Buff ──
    ...
    if (this.weaponRecoil > 0) {
      this.weaponRecoil = Math.max(
        0,
        this.weaponRecoil - this.weaponRecoilDecay * dt,
      );
    }
  ...
  _tickAnim(dt) {
    ...
    // state priority: dash > shoot > hurt > run > walk > idle
    if (A.dashT > 0.01) A.state = "dash";
    else if (A.shootT > 0.01) A.state = "shoot";
    else if (A.hurtT > 0.7) A.state = "hurt";
    else if (spd > 160) A.state = "run";
    else if (spd > 20) A.state = "walk";
    else A.state = "idle";
    // accumulate time in current state
    A.t += dt;
    // decay timers
    A.shootT = Math.max(0, A.shootT - dt * 5); // ~0.2s
    A.hurtT = Math.max(0, A.hurtT - dt * 3); // ~0.33s
    A.dashT = Math.max(0, A.dashT - dt * 4); // ~0.25s
    A.skillT = Math.max(0, A.skillT - dt);
    // ── Phase 4: lerp smoothing ─────────────────────────────
    const moveT = Math.min(1, spd / 180);
    const lerpK = 1 - Math.pow(0.01, dt * 8); // ~8 Hz convergence, dt-correct
    A.smoothMoveT = A.smoothMoveT + (moveT - A.smoothMoveT) * lerpK;
    // angle lerp — init on first tick to avoid spin from 0
    if (A.smoothAngle === null) A.smoothAngle = this.angle;
    // shortest-arc lerp to avoid 180° flip wrap
    let dA = this.angle - A.smoothAngle;
    while (dA > Math.PI) dA -= Math.PI * 2;
    while (dA < -Math.PI) dA += Math.PI * 2;
    A.smoothAngle += dA * lerpK;
  }
  ...
  breakStealth() {
    this.isInvisible = false;
    this.cooldowns.stealth =
      this.stats.stealthCooldown *
      this.cooldownMultiplier *
      this.skillCooldownMult;
  }
  ...
  checkPassiveUnlock() {
    ...
    if (
      ...
      this.level >= S.passiveUnlockLevel &&
      this.stealthUseCount >= S.passiveUnlockStealthCount
    ) {
  ...
  levelUp() {
    ...
    this.cooldownMultiplier = Math.max(
      0.5,
      this.cooldownMultiplier - cdReductionPerLevel,
    );
  ...
  applyDevBuff() {
    ...
    // ── Energy +50% ────────────────────────────────────────
    const enBonus = Math.floor(this.maxEnergy * 0.5);
    ...
    // ── Speed ×1.20 ────────────────────────────────────────
    this.metaSpeedMult *= 1.2;

    // ── CDR ×0.60 (stack กับ meta CDR ที่มีอยู่) ──────────
    this.skillCooldownMult = Math.max(0.1, this.skillCooldownMult * 0.6);
  ...
  updateUI() {
    ...
    if (this.isInvisible) {
      ...
      if (sCd) {
        const sp = Math.min(
          100,
          (1 - this.cooldowns.stealth / S.stealthCooldown) * 100,
        );
      ...
      if (typeof UIManager !== "undefined" && UIManager._setCooldownVisual) {
        UIManager._setCooldownVisual(
          "stealth-icon",
          Math.max(0, this.cooldowns.stealth),
          S.stealthCooldown,
        );
      }
    }
...
Player.prototype.checkObstacleProximity = function (ax, ay, dt, particleColor) {
  ...
  let mapObjs = [];
  if (window.mapSystem) {
    if (typeof mapSystem.getObjects === "function")
      mapObjs = mapSystem.getObjects();
    else if (Array.isArray(mapSystem.objects)) mapObjs = mapSystem.objects;
    else if (Array.isArray(mapSystem.solidObjects))
      mapObjs = mapSystem.solidObjects;
  }
  ...
  for (const obj of mapObjs) {
    ...
    const oL = obj.x,
      ...
      oR = oL + (obj.w || 0),
      oB = oT + (obj.h || 0);
    const halfW = (obj.w || 0) / 2,
      halfH = (obj.h || 0) / 2;
    if (Math.abs(this.x - (oL + halfW)) > warningThreshold + halfW + 10)
      continue;
    if (Math.abs(this.y - (oT + halfH)) > warningThreshold + halfH + 10)
      continue;
    ...
    if (d < warningThreshold && isMoving) {
      ...
      if (now - this.lastObstacleWarning > OB.obstacleWarningCooldown) {
        ...
        if (typeof UIManager !== "undefined")
          UIManager.showVoiceBubble("Careful!", this.x, this.y - 50);
      }
      break;
    }
  }
