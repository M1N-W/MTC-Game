...
class WanchaiStand {
  ...
  update(dt) {
    ...
    if (
      ...
      Math.hypot(
        this.x - this.ghostTrail[0]?.x,
        this.y - this.ghostTrail[0]?.y,
      ) > 8
    ) {
    ...
    if ((this._rushFistTimer ?? 0) > 0) {
      ...
      const fade = this._rushFistTimer / 0.1;
      if (this._rushFists)
        for (const f of this._rushFists) f.alpha = Math.max(0, fade);
    }
  ...
  _punch(target, owner) {
    ...
    // ── Heat gain per punch ────────────────────────────────
    if (typeof owner.gainHeat === "function")
      owner.gainHeat(S.heatPerHit ?? 12, true);
    ...
    let dmg =
      (S.wanchaiDamage ?? 24) * (owner.damageMultiplier || 1.0) * heatDmgMult;
    ...
    // Crit
    let critChance =
      (owner.baseCritChance ?? 0.06) + (S.standCritBonus ?? 0.18); // BUG-5 fix: was 0.40
    if (owner.passiveUnlocked) critChance += S.passiveCritBonus ?? 0;
    // OVERHEAT crit bonus
    if ((owner._heatTier ?? 0) >= 3)
      critChance += S.heatCritBonusOverheat ?? 0.12;
    ...
    if (owner.passiveUnlocked) {
      owner.hp = Math.min(
        owner.maxHp,
        owner.hp + dmg * (S.passiveLifesteal ?? 0.025),
      ); // BUG FIX: was hardcode 0.01 ≠ config 0.025
    }
    ...
    const _fanSpread = Math.max(
      Math.PI * 0.15,
      Math.PI * 0.38 - _oraCombo * 0.025,
    ); // tightens with combo
    ...
    for (let i = 0; i < _fanCount; i++) {
      ...
      this._rushFists.push({
        ox: Math.cos(a) * r,
        oy: Math.sin(a) * r,
        sc: 0.45 + t_ * 0.6, // small→big toward edge
        alpha: 1.0,
      });
    }
    ...
    if (target.hp !== undefined && target.hp <= 0 && owner.wanchaiActive) {
      ...
      owner.standMeter = Math.min(
        S2.standMeterMax ?? 100,
        (owner.standMeter ?? 0) + (S2.standMeterOnKill ?? 12),
      );

      // ── Feature 2: Killing Blow Supercharge from Stand punch ──
      if ((owner._oraComboCount ?? 0) >= (S2.oraComboSuperchargeMin ?? 5)) {
        if (typeof owner.gainHeat === "function")
          owner.gainHeat(S2.heatOnKillSupercharge ?? 30, true);
    ...
    if (owner.wanchaiActive) {
      ...
      owner.standMeter = Math.min(
        S3.standMeterMax ?? 100,
        (owner.standMeter ?? 0) + (S3.standMeterPerHit ?? 1),
      );
    }
  }
...
class AutoPlayer extends Player {
  constructor(x = 0, y = 0) {
    ...
    this.cooldowns = {
      ...(this.cooldowns || {}),
      dash: this.cooldowns?.dash ?? 0,
      stealth: this.cooldowns?.stealth ?? 0,
      shoot: 0,
      wanchai: 0,
      vacuum: 0,
      detonation: 0,
  ...
  takeDamage(amt) {
    ...
    const reduction = this.wanchaiActive
      ? (this.stats?.standDamageReduction ?? 0.3)
      : 0; // fix: was 0.5 ≠ config 0.30
    ...
    const rageReduction = this._rageMode
      ? (this.stats?.rageDamageReduction ?? 0.2)
      : 0;
  ...
  dealDamage(baseDamage) {
    ...
    if (this._rageMode) result.damage *= this.stats?.rageDamageMult ?? 1.3;
  ...
  _activateWanchai() {
    ...
    const dur =
      this.stats?.wanchaiDuration ??
      BALANCE.characters?.auto?.wanchaiDuration ??
      3.0;
    ...
    if (this._anim) this._anim.skillT = this.stats?.wanchaiDuration ?? 3.0;
  }
  ...
  gainHeat(amount, fromHit = false) {
    ...
    const newTier =
      this.heat >= (S.heatTierOverheat ?? 100)
        ? 3
        : this.heat >= (S.heatTierHot ?? 67)
          ? 2
          : this.heat >= (S.heatTierWarm ?? 34)
            ? 1
            : 0;
  ...
  update(dt, keys, mouse) {
    ...
    if (this._oraTextTimer > 0)
      this._oraTextTimer = Math.max(0, this._oraTextTimer - dt);
    ...
    // ── Heat Gauge tick ───────────────────────────────────────
    {
      ...
      const noDecayOnMove =
        ...
        (S.passiveHeatNoDecayOnMove ?? true) &&
        isMoving;
      const decayRate =
        this.wanchaiActive || noDecayOnMove
      ...
      if (prevTier >= 3 && newTier < 3) {
        ...
        if (window.boss && !window.boss.dead) {
          if (
            ...
            ventRange + (window.boss.radius ?? 0)
          ) {
            window.boss.takeDamage?.(
              ventDmg * 0.5 * (this.damageMultiplier || 1.0),
              this,
            );
          }
        }
      ...
      if (
        newTier === 0 &&
        prevWasCold &&
        typeof spawnFloatingText === "function"
      ) {
      ...
      const isLowHp = this.hp / this.maxHp <= (S.rageModeHpThreshold ?? 0.3);
    ...
    if (this.wanchaiActive) {
      ...
      if (this.standMeter <= 0) {
        ...
        const cd =
          this.stats?.wanchaiCooldown ??
          BALANCE.characters?.auto?.wanchaiCooldown ??
          12;
        this.cooldowns.wanchai = cd;
        if (this.wanchaiStand) {
          this.wanchaiStand.active = false;
          this.wanchaiStand = null;
        }
    ...
    if ((this._rushFistTimer ?? 0) > 0) {
      this._rushFistTimer -= dt;
      const _fade = Math.max(0, this._rushFistTimer / 0.1);
    ...
    if (checkInput("rightClick")) {
      ...
      if (
        !this.wanchaiActive &&
        (this.cooldowns?.wanchai ?? 0) <= 0 &&
        (this.energy ?? 0) >= energyCost
      ) {
    ...
    if (this.wanchaiActive)
      this.speedBoost =
        (this.speedBoost || 1) * (this.stats?.standSpeedMod ?? 1.5);

    // ── Feature 1: COLD tier — speed penalty ──────────────────
    if ((this._heatTier ?? 0) === 0) {
      const coldSpeedMult = this.stats?.coldSpeedMult ?? 0.9;
    ...
    if (
      ...
      if ((this.cooldowns?.vacuum ?? 0) <= 0) {
        ...
        spawnFloatingText(
          `⏳ ${Math.ceil(this.cooldowns.vacuum)}s`,
          this.x,
          this.y - 40,
          "#94a3b8",
          14,
    ...
    if (!this.passiveUnlocked) {
      ...
      if (eDown && !this._eHeld) {
        ...
        const S = this.stats ?? {};
        this._chargeTimer = Math.min(
          (this._chargeTimer ?? 0) + dt,
          S.chargeMaxTime ?? 1.5,
        );
        ...
        const chargeRatio = Math.min(
          1,
          (this._chargeTimer ?? 0) / (S.chargeMaxTime ?? 1.5),
        );
        ...
        if ((this.energy ?? 0) < detCost) {
          ...
          const DET_RANGE =
            (S.detonationRange ?? 240) * rangeMult * (isOverheat ? 1.5 : 1.0);
          const detBaseDmg =
            (S.detonationBaseDamage ?? 55) * dmgMult +
            this.heat * (S.detonationHeatScaling ?? 1.2);
          ...
          let detFinalDmg =
            detFinalBase * (detIsCrit ? (S.critMultiplier ?? 2.2) : 1.0);
          if (this.isSecondWind)
            detFinalDmg *= BALANCE.player.secondWindDamageMult || 1.5;
          ...
          for (const enemy of window.enemies || []) {
            ...
            if (Math.hypot(enemy.x - bx, enemy.y - by) < DET_RANGE) {
              ...
              spawnParticles(
                ...
                enemy.y,
                5,
                detIsCrit ? "#facc15" : "#dc2626",
              );
            }
          }
          if (window.boss && !window.boss.dead) {
            if (
              Math.hypot(window.boss.x - bx, window.boss.y - by) <
              DET_RANGE + (window.boss.radius ?? 0)
            ) {
              const bossPx = window.boss.x,
                bossPy = window.boss.y; // FIX: snapshot ก่อน takeDamage — boss อาจ null หลัง call
          ...
          if (this.passiveUnlocked && totalDet > 0) {
            this.hp = Math.min(
              this.maxHp,
              this.hp + totalDet * (S.passiveLifesteal ?? 0.025),
            );
          }
          ...
          const chargeLabel =
            chargeRatio >= 0.99
              ? "💥 MAX CHARGE!"
              : detIsCrit
                ? `💥 CRIT! ${Math.floor(detFinalDmg)}`
                : `💥 ${Math.floor(detFinalDmg)}`;
          spawnFloatingText(
            chargeLabel,
            bx,
            by - 70,
            detIsCrit ? "#facc15" : "#dc2626",
            chargeRatio >= 0.99 ? 34 : 28,
          );
          if (typeof Audio !== "undefined" && Audio.playDetonation)
      ...
      if (checkInput("e")) {
        if ((this.cooldowns?.detonation ?? 0) > 0) {
          spawnFloatingText(
            `⏳ ${Math.ceil(this.cooldowns.detonation)}s`,
            this.x,
            this.y - 40,
            "#94a3b8",
            14,
    ...
    if (this.wanchaiActive) {
      ...
      if ((this.cooldowns?.shoot ?? 0) <= 0) {
        ...
        this.cooldowns.shoot = this.stats?.playerRushCooldown ?? 0.1;
  ...
  _doPlayerMelee(mouse) {
    ...
    if (this.wanchaiStand?.active) {
      const _sd = Math.hypot(
        target.x - this.wanchaiStand.x,
        target.y - this.wanchaiStand.y,
      );
      if (_sd > 1) {
        const _rushSpeed = 2200; // px/s burst — fast but not instant
        this.wanchaiStand._rushBurstVx =
          ((target.x - this.wanchaiStand.x) / _sd) * _rushSpeed;
        this.wanchaiStand._rushBurstVy =
          ((target.y - this.wanchaiStand.y) / _sd) * _rushSpeed;
        this.wanchaiStand._rushBurstTimer = 0.18;
        this.wanchaiStand.angle = Math.atan2(
          target.y - this.wanchaiStand.y,
          target.x - this.wanchaiStand.x,
        );
      }
      ...
      pht >= 3
        ...
        : pht >= 2
          ...
          : pht >= 1
            ...
            : (S.coldDamageMult ?? 0.75); // COLD penalty
    let dmg =
      (S.wanchaiDamage ?? 24) * (this.damageMultiplier || 1.0) * pHeatMult;
    ...
    if (this.passiveUnlocked)
      this.hp = Math.min(
        this.maxHp,
        this.hp + dmg * (S.passiveLifesteal ?? 0.025),
      ); // BUG-4 fix: was 0.02
    ...
    this.cooldowns.shoot = Math.max(
      0.04,
      (this.stats?.playerRushCooldown ?? 0.1) - _comboSpeedBonus,
    );
    ...
    const _hSpread = Math.max(
      Math.PI * 0.15,
      Math.PI * 0.38 - this._oraComboCount * 0.025,
    );
    ...
    if (this.wanchaiStand) {
      ...
      this.wanchaiStand._phaseTimer = 0.1;
    }

    if (typeof Achievements !== "undefined" && target.hp <= 0) {
      Achievements.stats.standRushKills =
        (Achievements.stats.standRushKills ?? 0) + 1;
      Achievements.check?.("stand_rush_kill");
    }

    // ── Feature 4: Stand Meter fill on hit ──────────────────
    const S_meter = this.stats ?? {};
    this.standMeter = Math.min(
      S_meter.standMeterMax ?? 100,
      (this.standMeter ?? 0) + (S_meter.standMeterPerHit ?? 1),
    );

    // ── Feature 2: Killing Blow Supercharge ─────────────────
    if (
      target.hp <= 0 &&
      (this._oraComboCount ?? 0) >= (S_meter.oraComboSuperchargeMin ?? 5)
    ) {
  ...
  checkPassiveUnlock() {
    ...
    if (typeof Achievements !== "undefined")
      Achievements.check("scorched_soul");
  ...
  _doStandPull() {
    ...
    if (window.boss && !window.boss.dead) {
      ...
      if (d < pullRange && d > 1) {
        const pullAngle = Math.atan2(
          window.boss.y - standY,
          window.boss.x - standX,
        );
        const bossOffset = (window.boss.radius ?? 30) + 50;
        window.boss.vx =
          (standX + Math.cos(pullAngle) * bossOffset - window.boss.x) * 8;
        window.boss.vy =
          (standY + Math.sin(pullAngle) * bossOffset - window.boss.y) * 8;
      }
    }
    ...
    if (pulled > 0) {
      ...
      if (typeof spawnParticles === "function")
        spawnParticles(standX, standY, 35, "#dc2626");
      if (typeof addScreenShake === "function") addScreenShake(5);
      if (typeof spawnFloatingText === "function")
        spawnFloatingText(
          `🥊 STAND PULL ×${pulled}`,
  ...
  _doVacuum({ earlyMode = false } = {}) {
    ...
    const STUN_DUR = S.vacuumStunDur ?? 0.5;
    ...
    const heatGain = earlyMode
      ? (S.vacuumEarlyHeatGain ?? 10)
      : (S.vacuumHeatGain ?? 25);
    ...
    if (window.boss && !window.boss.dead) {
      ...
      const d = Math.hypot(dx, dy);
      if (d < VACUUM_RANGE && d > 1) {
        window.boss.vx = (dx / d) * VACUUM_FORCE * 0.3;
        window.boss.vy = (dy / d) * VACUUM_FORCE * 0.3;
  ...
  updateUI() {
    ...
    const wanchaiCd =
      S.wanchaiCooldown || BALANCE.characters?.auto?.wanchaiCooldown || 12;
    ...
    if (standEl)
      standEl.style.boxShadow = this.wanchaiActive
    ...
    if (this.wanchaiActive) {
      ...
      if (typeof UIManager !== "undefined" && UIManager._setCooldownVisual) {
        UIManager._setCooldownVisual(
          "stealth-icon",
          Math.max(0, this.cooldowns.wanchai ?? 0),
          wanchaiCd,
        );
      }
    }
    ...
    // ── Heat Gauge HUD ──────────────────────────────────────────
    {
      ...
      if (heatLabelEl) {
        const label =
          ht >= 3 ? "OVERHEAT" : ht >= 2 ? "HOT" : ht >= 1 ? "WARM" : "HEAT";
    ...
    if (typeof UIManager !== "undefined" && UIManager._setCooldownVisual) {
      ...
      UIManager._setCooldownVisual(
        "vacuum-icon",
        Math.max(0, this.cooldowns.vacuum ?? 0),
        vacMaxCd,
      );
      UIManager._setCooldownVisual(
        "auto-det-icon",
        Math.max(0, this.cooldowns.detonation ?? 0),
