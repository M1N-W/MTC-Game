...
class FloatingText {
  ...
  _category() {
    ...
    if (
      isNumeric &&
      (col === RT.palette.crit || col === RT.palette.gold || this.size >= 32)
    )
  ...
  draw() {
    ...
    switch (cat) {
      case "CRIT":
        this._drawCrit(screen);
        break;
      case "DAMAGE":
        this._drawDamage(screen);
        break;
      case "HEAL":
        this._drawHeal(screen);
        break;
      case "IMPACT":
        this._drawImpact(screen);
        break;
      case "STATUS":
        this._drawStatus(screen);
        break;
      default:
        this._drawDefault(screen);
        break;
    }
    CTX.restore();
  }
  ...
  _drawCrit(sc) {
    ...
    CTX.shadowBlur = 28;
    CTX.shadowColor = RT.palette.critGlow;
    ...
    grd.addColorStop(0, RT.palette.critTop);
    grd.addColorStop(0.4, RT.palette.crit);
    grd.addColorStop(1, RT.palette.critGlow);
    CTX.fillStyle = grd;
  ...
  _drawHeal(sc) {
    ...
    CTX.fill();
    CTX.strokeStyle = RT.palette.healBorder;
    CTX.lineWidth = 1.5;
    CTX.stroke();
    CTX.shadowBlur = 10;
  ...
  _drawDefault(sc) {
    ...
    CTX.font =
      this._font ||
      (this._font = `bold ${this.size}px 'Rajdhani', 'Orbitron', monospace`);
    CTX.shadowBlur = 12;
    CTX.shadowColor = this.color;
    CTX.lineWidth = sz * 0.12;
    CTX.strokeStyle = "rgba(0,0,0,0.9)";
...
class FloatingTextSystem {
  ...
  spawn(text, x, y, color, size = 20) {
    ...
    this.texts.push(
      FloatingText.acquire(text, x, y - stack * STEP_Y, color, size)
    );
  }
  update(dt) {
...
class HitMarker {
  ...
  draw() {
    ...
    CTX.save();
    CTX.globalAlpha = alpha;
    CTX.strokeStyle = this.isCrit ? RT.palette.crit : "#ffffff";
...
function spawnWanchaiPunchText(x, y) {
  ...
  const label =
    WANCHAI_PUNCH_LABELS[
      Math.floor(Math.random() * WANCHAI_PUNCH_LABELS.length)
    ];
  const offX = (Math.random() - 0.5) * 40;
  const offY = -10 + (Math.random() - 0.5) * 20;
  // RT tokens: dangerDark + bossOrange + dangerSoft match the Thermodynamic Brawler theme
  const colours = [
