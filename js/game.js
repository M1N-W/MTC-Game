...
function _tutorialForwardInput() {
  ...
  if (GameState.isSlowMotion && !_tut._prevBulletTime)
...
function updateGame(dt) {
  ...
  if (window.player.dead) {
    if (
      typeof decalSystem !== "undefined" &&
      !window.player._deathDecalSpawned
    ) {
  ...
  if (GameState.isGlitchWave) {
    GameState.glitchIntensity = Math.min(
      1.0,
      GameState.glitchIntensity + GLITCH_RAMP * dt
    );
  } else {
    GameState.glitchIntensity = Math.max(
      0.0,
      GameState.glitchIntensity - GLITCH_RAMP * 2 * dt
    );
  }

  const _inTutorial =
    typeof TutorialSystem !== "undefined" && TutorialSystem.isActive();
...
function _tickWaveEvents(dt) {
  ...
  if (GameState.waveSpawnLocked) {
    ...
    if (
      secsLeft !== GameState.lastGlitchCountdown &&
      secsLeft > 0 &&
      secsLeft <= 3
    ) {
      GameState.lastGlitchCountdown = secsLeft;
      spawnFloatingText(
        GAME_TEXTS.wave.spawnCountdown(secsLeft),
        window.player.x,
        window.player.y - 145,
        "#d946ef",
        34
  ...
  dayNightTimer += dt;
  {
    ...
    L.ambientLight =
      L.nightMinLight + dayPhase * (L.dayMaxLight - L.nightMinLight);
  }
}
...
function _tickShopBuffs(dt) {
  ...
  if (p.shopDamageBoostActive) {
    ...
    if (p.shopDamageBoostTimer <= 0) {
      ...
      p.damageBoost =
        p._baseDamageBoost !== undefined ? p._baseDamageBoost : 1.0;
      spawnFloatingText(
        GAME_TEXTS.shop.dmgBoostExpired,
        p.x,
        p.y - 50,
        "#94a3b8",
        14
  ...
  if (p.shopSpeedBoostActive) {
    ...
    if (p.shopSpeedBoostTimer <= 0) {
      ...
      spawnFloatingText(
        GAME_TEXTS.shop.spdBoostExpired,
        p.x,
        p.y - 50,
        "#94a3b8",
        14
...
function _checkProximityInteractions(_inTutorial) {
  if (
    ...
    typeof ShopManager.tick === "function"
  )
    ShopManager.tick();

  const dToServer = dist(
    window.player.x,
    window.player.y,
    MTC_DATABASE_SERVER.x,
    MTC_DATABASE_SERVER.y
  );
  if (
    !_inTutorial &&
    dToServer < MTC_DATABASE_SERVER.INTERACTION_RADIUS &&
    keys.e === 1
  ) {
    keys.e = 0;
  ...
  if (
    !_inTutorial &&
    dToShop < MTC_SHOP_LOCATION.INTERACTION_RADIUS &&
    keys.b === 1
  ) {
    keys.b = 0;
    openShop();
...
function _tickEntities(dt, _inTutorial) {
  ...
  if (
    ...
    !(typeof AutoPlayer === "function" && window.player instanceof AutoPlayer)
  ) {
    ...
    const isPoom =
      typeof PoomPlayer !== "undefined" && window.player instanceof PoomPlayer;
    const isPat =
      typeof PatPlayer !== "undefined" && window.player instanceof PatPlayer;
    if (isKao) {
      if (effectiveMouse.left === 1 && GameState.phase === "PLAYING")
        window.player.shoot(dt);
    } else if (
      !isPoom &&
      !isPat &&
      effectiveMouse.left === 1 &&
  ...
  if (typeof WorkerBridge !== "undefined") {
    ...
    typeof window.playerAnalyzer !== "undefined" &&
    window.boss &&
    !window.boss.dead
  ) {
  ...
  if (window.boss && !window.boss.dead) {
    ...
    if (_safeRoom) {
      ...
      const _entryPad = 28,
        _exitPad = 90;
      const _inRoom =
        _bx > _safeRoom.x - _entryPad &&
        _bx < _safeRoom.x + _safeRoom.w + _entryPad &&
        _by > _safeRoom.y - _entryPad &&
        _by < _safeRoom.y + _safeRoom.h + _entryPad;
      const _clearOut =
        _bx < _safeRoom.x - _exitPad ||
        _bx > _safeRoom.x + _safeRoom.w + _exitPad ||
        _by < _safeRoom.y - _exitPad ||
        _by > _safeRoom.y + _safeRoom.h + _exitPad;
      if (_inRoom) {
        ...
        if (!window.boss._inSafeZone) {
          ...
          spawnFloatingText(
            GAME_TEXTS.environment?.safeZone ?? "SAFE ZONE",
            window.boss.x,
            window.boss.y - 60,
            "#22d3ee",
            22
  ...
  if (!_inTutorial) {
    ...
    if (typeof window.squadAI !== "undefined")
      window.squadAI.update(dt, window.enemies, window.player);
  ...
  if (
    !_inTutorial &&
    (window.WAVE_SCHEDULE?.bossWaves
      ? !window.WAVE_SCHEDULE.bossWaves.includes(getWave())
      : true) &&
    window.enemies.length === 0 &&
    !window.boss &&
    !GameState.waveSpawnLocked &&
    !window.isTrickleActive
  ) {
    if (
      Achievements.stats.damageTaken === GameState.waveStartDamage &&
...
function _tickBarrelExplosions() {
  if (
    typeof projectileManager === "undefined" ||
    !projectileManager ||
    !projectileManager.projectiles ||
    !Array.isArray(projectileManager.projectiles) ||
    !window.mapSystem ||
    !Array.isArray(window.mapSystem.objects)
  )
    return;
  ...
  for (let pi = allProj.length - 1; pi >= 0; pi--) {
    ...
    for (let bi = window.mapSystem.objects.length - 1; bi >= 0; bi--) {
      ...
      if (
        Math.hypot(proj.x - closestX, proj.y - closestY) < (proj.radius || 6)
      ) {
  ...
  for (const obj of window.mapSystem.objects) {
    ...
    if (obj.hp <= 0 && !obj.isExploded) {
      ...
      spawnFloatingText(
        GAME_TEXTS.environment?.barrelBoom ?? "💥 BOOM!",
        bCX,
        bCY - 55,
        "#f97316",
        38
      ...
      if (window.boss && !window.boss.dead) {
        ...
        if (bd < EXPL_R) {
          ...
          spawnFloatingText(
            GAME_TEXTS.environment?.barrelHit ?? "BARREL HIT!",
            window.boss.x,
            window.boss.y - 80,
            "#f97316",
            26
...
function _tickEnvironment(dt, _inTutorial) {
  for (let i = window.specialEffects.length - 1; i >= 0; i--) {
    const remove = window.specialEffects[i].update(
      dt,
      window.player,
      window.meteorZones,
      window.boss
    ...
    if (remove) {
      window.specialEffects[i] =
        window.specialEffects[window.specialEffects.length - 1];
  ...
  if (
    ...
    for (let i = 0; i < window.specialEffects.length; i++) {
      ...
      fx.startY = newStartY;
      fx.angle = Math.atan2(
        window.boss.y - newStartY,
        window.boss.x - newStartX
      );
      fx.length = 0;
      fx.phase = "expanding";
      fx.timer = 0;
      ...
      spawnFloatingText(
        GAME_TEXTS.combat?.graphParried ?? "⚔ GRAPH PARRIED!",
        window.player.x,
        window.player.y - 70,
        "#facc15",
        28
      ...
      if (typeof Audio !== "undefined" && Audio.playReflect)
        Audio.playReflect();
      addScreenShake(8);
      ...
      if (window.player._perfectParryArmed) {
        ...
        window.player._invincibleTimer =
          window.player.stats.perfectParryIFrameDur ?? 0.4;
        window.player.energy = Math.min(
          window.player.maxEnergy ?? 100,
          (window.player.energy ?? 0) + (patS.perfectParryEnergyRestore ?? 20)
        );
        spawnFloatingText(
          GAME_TEXTS.combat?.perfectParry ?? "⚔ PERFECT!",
          window.player.x,
          window.player.y - 100,
          "#ff4500",
          22
  ...
  for (let i = window.meteorZones.length - 1; i >= 0; i--) {
    ...
    if (
      !_inTutorial &&
      dist(
        window.meteorZones[i].x,
        window.meteorZones[i].y,
        window.player.x,
        window.player.y
      ) < window.meteorZones[i].radius
    ) {
  ...
  if (window.player && !window.player.dead)
    _mapUpdateEntities.push(window.player);
...
function drawGame() {
  ...
  if (DEBUG_MODE && drawGame._diagFrame % 300 === 1) {
    console.log(
      ...
      "| UIManager:",
      typeof UIManager,
      "| MTC_DB_SERVER on window:",
      !!window.MTC_DATABASE_SERVER,
      "| MTC_SHOP on window:",
      !!window.MTC_SHOP_LOCATION
    );
  }
  ...
  if (
    !_bgGrad ||
    _bgGradW !== CANVAS.width ||
    _bgGradH !== CANVAS.height ||
    _bgGradTop !== _bgTop ||
    _bgGradBot !== _bgBot
  ) {
  ...
  if (window.boss && !window.boss.dead && window.boss.isOnScreen(200))
    BossRenderer.draw(window.boss, CTX);
  ...
  {
    const allProj =
      typeof projectileManager !== "undefined" && projectileManager.projectiles
        ? projectileManager.projectiles
        : [];
    CTX.save();
  ...
  if (typeof GravitationalSingularity !== "undefined")
    GravitationalSingularity.draw(CTX);
...
function drawDayNightHUD() {
  const L = BALANCE.LIGHTING;
  const phase =
    (L.ambientLight - L.nightMinLight) / (L.dayMaxLight - L.nightMinLight);
  ...
  CTX.strokeStyle = isDawn
    ? "rgba(255, 210, 80, 0.55)"
    : "rgba(130, 160, 255, 0.55)";
  CTX.lineWidth = 2;
  CTX.beginPath();
  ...
  CTX.fillText(
    isDawn ? GAME_TEXTS.ui.dayPhase(pct) : GAME_TEXTS.ui.nightPhase(100 - pct),
    cx,
    cy + r + 14
  );
  CTX.restore();
...
function drawGrid() {
  ...
  if (
    ...
    oy !== _gridOy ||
    CANVAS.width !== _gridW ||
    CANVAS.height !== _gridH
  ) {
    _gridPath = new Path2D();
    for (let x = ox; x < CANVAS.width; x += sz) {
      _gridPath.moveTo(x, 0);
      _gridPath.lineTo(x, CANVAS.height);
    }
    for (let y = oy; y < CANVAS.height; y += sz) {
      _gridPath.moveTo(0, y);
      _gridPath.lineTo(CANVAS.width, y);
    }
    _gridOx = ox;
    _gridOy = oy;
    _gridW = CANVAS.width;
    _gridH = CANVAS.height;
  }
...
function _createPlayer(charType) {
  ...
  if (window.isAdmin && typeof player.applyDevBuff === "function") {
    player.applyDevBuff();
    console.log(
      "%c[MTC Admin] 🚀 DEV BUFF applied to player.",
...
function endGame(result) {
  ...
  if (result === "victory") {
    ...
    setElementText(
      "final-kills",
      `${(Achievements.stats.kills || 0).toLocaleString()}`
    );
