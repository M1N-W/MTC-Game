...
function startNextWave() {
    ...
    // ── BGM: switch to battle music for normal waves, glitch for glitch waves ──────────────
    // Boss music is handled later, inside the bossEveryNWaves setTimeout block
    // when the boss actually spawns.  Normal waves use battle BGM, glitch waves use glitch BGM.
    // This also restores appropriate music after a boss wave ends.
    if (getWave() % BALANCE.waves.bossEveryNWaves !== 0) {
        if (isGlitchWave) {
