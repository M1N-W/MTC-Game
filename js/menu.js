 ...
 *  UIManager.js   — window.PORTRAITS (SVG strings per char)
 ...
 *  ⚠️  Reads window.PORTRAITS — safe only after UIManager.js executes.
...
// ── Portrait injection — window.load fires AFTER UIManager.js executes ──
// This guarantees window.PORTRAITS (defined in UIManager.js) is ready before we try
// to populate the char-select cards and the default HUD portrait.
window.addEventListener("load", function _injectPortraits() {
  if (!window.PORTRAITS) {
    // UIManager.js wasn't loaded yet — shouldn't happen, but don't crash
    console.warn(
...
(function _initSkillTooltips() {
  ...
  function _init() {
    ...
    cards.forEach((card) => {
      ...
      card.addEventListener("touchend", () => clearTimeout(_holdTimer), {
        passive: true,
      });
      card.addEventListener("touchcancel", () => clearTimeout(_holdTimer), {
        passive: true,
      });
    });
    ...
    function _showTooltip(tooltip, card) {
      ...
      const left = Math.max(
        8,
        Math.min(cardRect.left + cardRect.width / 2 - w / 2, VW - w - 8)
      );
