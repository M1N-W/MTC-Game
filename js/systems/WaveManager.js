...
function _getWaveSchedule() {
  ...
  return {
    ...
    dark: new Set(
      Array.isArray(ws.darkWave) ? ws.darkWave : [ws.darkWave ?? 1]
    ),
...
function _activateWaveEvent(type, wave) {
  ...
  if (window.player && window.UIManager) {
    const voiceLine =
      type === "fog" ? "⚠ Radar offline..." : "⚡ They're moving fast!";
...
function _patchEnemySpeeds() {
  ...
  for (const e of all) {
    if (!e || e.dead || _patchedEnemies.has(e) || typeof e.speed !== "number")
      continue;
...
function _drawDark(ctx) {
  ...
  const flicker =
    0.82 + Math.sin(now * 1.3) * 0.04 + Math.sin(now * 3.7) * 0.02;
...
function _drawFog(ctx) {
  ...
  if (a > 0.3) {
    ...
    ctx.beginPath();
    ctx.moveTo(rx - 36, ry - 36);
    ctx.lineTo(rx + 36, ry + 36);
    ctx.moveTo(rx + 36, ry - 36);
    ctx.lineTo(rx - 36, ry + 36);
    ctx.stroke();
  }
...
function _drawSpeed(ctx) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const len = 55 + (i % 4) * 35;
    const x =
      ((t * W * 0.4 * (i % 2 === 0 ? 1 : -1) + W * 1.2) % (W * 1.4)) - W * 0.2;
    const dir = i % 2 === 0 ? 1 : -1;
...
function startNextWave() {
  ...
  const wave = getWave();
  const count =
    BALANCE.waves.enemiesBase + (wave - 1) * BALANCE.waves.enemiesPerWave;
...
function _startGlitchWave(count) {
  ...
  setTimeout(() => {
    if (
      ...
      spawnFloatingText(
        GAME_TEXTS.wave.glitchControls,
        window.player.x,
        window.player.y - 158,
        "#f472b6",
        28
...
function _startBossWave(wave) {
  ...
  setTimeout(() => {
    ...
    if (typeof GameState !== "undefined")
      GameState.bossEncounterCount = window.bossEncounterCount;
    ...
    const displayLevel = isFirst
      ? Math.floor(encounter / 2)
      : Math.ceil(encounter / 2);
    ...
    if (isFirst) {
      ...
      setTimeout(() => {
        if (window.player)
          spawnFloatingText(
            isAdvanced
              ? GAME_TEXTS.wave.firstTaglineAdvanced
              : GAME_TEXTS.wave.firstTagline,
            window.player.x,
            window.player.y - 148,
            "#86efac",
            22
      ...
      spawnFloatingText(
        enablePhase3
          ? GAME_TEXTS.wave.bossIncomingFish
          : enablePhase2
          ? GAME_TEXTS.wave.bossIncomingRider
...
function spawnEnemies(count) {
  for (let i = 0; i < count; i++) {
    ...
    const hf = window.WAVE_SCHEDULE?.healerFirstWave ?? 999;
    const sf = window.WAVE_SCHEDULE?.sniperFirstWave ?? 999;
    const healerChance = wave >= hf ? BALANCE.waves.healerSpawnChance ?? 0 : 0;
    const sniperChance = wave >= sf ? BALANCE.waves.sniperSpawnChance ?? 0 : 0;
    const mageChance = BALANCE.waves.mageSpawnChance ?? 0;
    const tankChance = BALANCE.waves.tankSpawnChance ?? 0;

    const r = Math.random();
    let spawnKind = "basic";
    if (r < healerChance) spawnKind = "healer";
    else if (r < healerChance + sniperChance) spawnKind = "sniper";
    else if (r < healerChance + sniperChance + mageChance) spawnKind = "mage";
    else if (r < healerChance + sniperChance + mageChance + tankChance)
      spawnKind = "tank";
    ...
    const spawnRadius =
      spawnKind === "tank"
        ...
        ? BALANCE.mage.radius
        : spawnKind === "healer"
        ? BALANCE.healer.radius
        : spawnKind === "sniper"
        ? BALANCE.sniper.radius
        : BALANCE.enemy.radius;
    const safe = mapSystem.findSafeSpawn(x, y, spawnRadius);
    x = safe.x;
    y = safe.y;

    if (spawnKind === "healer") window.enemies.push(new HealerEnemy(x, y));
    else if (spawnKind === "sniper") window.enemies.push(new SniperEnemy(x, y));
    else if (spawnKind === "mage") window.enemies.push(new MageEnemy(x, y));
    else if (spawnKind === "tank") window.enemies.push(new TankEnemy(x, y));
