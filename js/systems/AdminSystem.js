...
const MTC_DATABASE_SERVER = {
  get x() {
    return BALANCE.database.x;
  },
  get y() {
    return BALANCE.database.y;
  },
  get INTERACTION_RADIUS() {
    return BALANCE.database.interactionRadius;
  },
};
...
const AdminConsole = (() => {
  ...
  function _killAllEntities() {
    ...
    if (
      window.boss &&
      !window.boss.dead &&
      typeof window.boss.takeDamage === "function"
    ) {
  ...
  function _parse(raw) {
    ...
    _appendLine(
      `[${String(_cmdCount).padStart(3, "0")}] ${permInfo.badge}${
        permInfo.text
      }@mtcserver:~# ${raw}`,
      "cline-cmd",
      true
    ...
    if (ROOT_ONLY.includes(base) && perm < PERM.ROOT) {
      ...
      _appendLine(
        `   Your level: ${PERM_LABEL[perm].text}  — contact system admin.`,
        "cline-warn",
        true
      );
      return;
    ...
    if (base === "heal") {
      ...
      _appendLine(
        GAME_TEXTS.admin.healResult(Math.round(window.player.hp), maxHp),
        "cline-ok"
      );
      if (typeof spawnFloatingText === "function")
        spawnFloatingText(
          GAME_TEXTS.admin.healFloat(gained),
          window.player.x,
          window.player.y - 70,
          "#00ff41",
          22
    ...
    // ══════════════════════════════════════════════════════
      ...
      _appendLine(
        GAME_TEXTS.admin.scoreResult(
          typeof getScore === "function" ? getScore().toLocaleString() : "?"
        ),
        "cline-ok"
      );
    ...
    // ══════════════════════════════════════════════════════
      ...
      const maxWave =
        (typeof BALANCE !== "undefined" && BALANCE.waves?.maxWaves) || 15;
      if (num > maxWave) {
        _appendLine(
          `ERR: wave ${num} exceeds maxWaves (${maxWave})`,
          "cline-error"
        );
        return;
      }
      ...
      if (typeof startNextWave === "function") {
        startNextWave();
        _appendLine(
          `✔ Jumped to Wave ${num}. Enemies spawning now.`,
          "cline-ok"
        );
      } else {
        _appendLine(
          "WARN: startNextWave() not found. Wave counter set but not triggered.",
          "cline-warn",
          true
        );
        _appendLine(
          `✔ Wave counter = ${num - 1}. Will advance on next game-loop cycle.`,
          "cline-ok"
        );
      }
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (!weaponName) {
        ...
        _appendLine(
          "> Available: pistol, rifle, shotgun, sniper, smg, launcher",
          "cline-info",
          true
        );
        return;
      ...
      if (typeof window.player.switchWeapon === "function") {
        try {
          ...
          _appendLine(
            `ERR: switchWeapon threw — ${err.message}`,
            "cline-error"
          );
        }
      }
      // Fallback: weaponSystem.give / setWeapon
        ...
        const fn =
          window.weaponSystem.give ||
          window.weaponSystem.setWeapon ||
          window.weaponSystem.switchWeapon;
        if (typeof fn === "function") {
          try {
            fn.call(window.weaponSystem, weaponName);
            success = true;
          } catch (err) {
            _appendLine(`ERR: weaponSystem — ${err.message}`, "cline-error");
          }
        }
      }
      if (success) {
        ...
        _appendLine(
          `ERR: weapon "${weaponName}" not found or equip API unavailable.`,
          "cline-error"
        );
        _appendLine(
          "> Try: pistol, rifle, shotgun, sniper, smg, launcher",
          "cline-info",
          true
        );
      }
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (isManop && typeof ManopBoss === "undefined") {
        _appendLine(
          "ERR: ManopBoss class not found. Is ManopBoss.js loaded?",
          "cline-error"
        );
        return;
      }
      if (!isManop && typeof KruFirst === "undefined") {
        _appendLine(
          "ERR: KruFirst class not found. Is FirstBoss.js loaded?",
          "cline-error"
        );
        return;
      }
      ...
      if (window.boss && !window.boss.dead)
        _appendLine(
          "WARN: existing boss detected — overwriting reference.",
          "cline-warn",
          true
        );
      ...
      if (isManop) {
        ...
        if (phase < 1 || phase > 3) {
          ...
          _appendLine(
            ">  phase 2 = Dog Rider  (enablePhase2)",
            "cline-info",
            true
          );
          _appendLine(
            ">  phase 3 = Goldfish Lover  (enablePhase2 + enablePhase3)",
            "cline-info",
            true
          );
          return;
        ...
        _appendLine(GAME_TEXTS.admin.authOk, "cline-info");
        _appendLine(
          `> Allocating Boss: KRU MANOP  Phase ${phase}...`,
          "cline-info"
        );
        ...
        const phaseLabel =
          phase === 3
            ...
            : "💢 BASIC";
        _appendLine(
          `✔ KRU MANOP  ${phaseLabel}  spawned (difficulty ×${difficulty}).`,
          "cline-ok"
        );
        spawnFloatingText(
          phase === 3
            ? "🐟 KRU MANOP — GOLDFISH LOVER!"
            : phase === 2
            ? "🐕 KRU MANOP — DOG RIDER!"
            : "💢 KRU MANOP APPEARS!",
          window.player.x,
          window.player.y - 80,
          phase === 3 ? "#38bdf8" : phase === 2 ? "#d97706" : "#ef4444",
          26
        );
      } else {
        ...
        if (arg3 && !isAdvanced) {
          ...
          _appendLine(
            ">  advanced = encounter 4 variant (+35% stats)",
            "cline-info",
            true
          );
          return;
        ...
        _appendLine(GAME_TEXTS.admin.authOk, "cline-info");
        _appendLine(
          `> Allocating BossFirst: KRU FIRST  ${
        ...
        _appendLine(
          `✔ KRU FIRST  ${
            isAdvanced ? "⚛️ ADVANCED" : "Normal"
          }  spawned (difficulty ×${difficulty}).`,
          "cline-ok"
        );
        spawnFloatingText(
      ...
      if (typeof window.UIManager !== "undefined")
        window.UIManager.updateBossHUD(window.boss);
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (typeof window.player.applyDevBuff !== "function") {
        _appendLine(
          "ERR: applyDevBuff() not found. Is PlayerBase.js updated?",
          "cline-error"
        );
        return;
      }
      if (window.player._devBuffApplied) {
        _appendLine(
          "WARN: Dev buff already active on this player.",
          "cline-warn",
          true
        );
        _appendLine(
          `  HP ${Math.round(window.player.hp)}/${
            window.player.maxHp
          }  EN ${Math.round(window.player.energy)}/${window.player.maxEnergy}`,
          "cline-info",
          true
        );
        return;
      ...
      _appendLine(
        `  DMG ×1.25 | SPD ×1.20 | CDR ×0.60 | Crit +8%`,
        "cline-ok",
        true
      );
      _appendLine(
        "  Passive skills: unchanged (unlock normally in-game)",
        "cline-info",
        true
      );
      if (typeof spawnFloatingText === "function")
    ...
    // ══════════════════════════════════════════════════════
      ...
      _appendLine(
        `✔ Energy: ${Math.round(window.player.energy)}/${maxE}`,
        "cline-ok"
      );
      if (typeof spawnFloatingText === "function")
        spawnFloatingText(
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (sub === "off" || _godMode) {
        ...
        _appendLine(
          '✔ GOD MODE ACTIVE  (type "god off" to disable)',
          "cline-ok"
        );
        if (typeof spawnFloatingText === "function")
          spawnFloatingText(
    ...
    // ══════════════════════════════════════════════════════
      ...
      const killed = _killAllEntities();
      _appendLine(
        `✔ ${killed} entit${killed === 1 ? "y" : "ies"} terminated.`,
        "cline-ok"
      );
      if (typeof spawnFloatingText === "function")
        spawnFloatingText(
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (isNaN(mult) || mult <= 0 || mult > 10) {
        _appendLine(
          "ERR: usage: speed <multiplier>  (0.1 – 10)",
          "cline-error"
        );
        return;
      }
      ...
      _appendLine(
        `✔ Speed set to ×${mult}  (base: ${window.player._origSpeed.toFixed(
          1
        )})`,
        "cline-ok"
      );
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (overlay) {
        ...
        _appendLine(
          `✔ FPS overlay ${visible ? "hidden" : "shown"}.`,
          "cline-ok"
        );
      } else {
    ...
    // ══════════════════════════════════════════════════════
      ...
      const p = window.player;
      const wave =
        typeof getWave === "function" ? getWave() : window.wave || "?";
      const score =
        typeof getScore === "function" ? getScore() : window.score || 0;
      const enemyCnt = (window.enemies || []).filter(
        (e) => e && !e.dead
      ).length;
      ...
      const infoLines = [
        ...
        `  Boss      : ${
          bossAlive ? window.boss.constructor?.name || "ACTIVE" : "none"
        }`,
      ...
      infoLines.forEach((l, i) =>
        setTimeout(() => _appendLine(l, "cline-ok", true), i * 25)
      );
    }
    ...
    // ══════════════════════════════════════════════════════
      ...
      helpLines.forEach((l, i) => {
        const cls =
          l.includes("★") && pLvl < PERM.ROOT ? "cline-sys" : "cline-info";
    ...
    // ══════════════════════════════════════════════════════
      ...
      trimmed.toLowerCase() === "sudo rm -rf /" ||
      trimmed.toLowerCase() === "sudo rm -rf *"
    ) {
      ...
      _appendLine(
        `  Permission: ${pInfo.badge} ${pInfo.text}`,
        "cline-ok",
        true
      );
    } else if (base === "ls") {
      GAME_TEXTS.admin.lsEntries.forEach((e) =>
        _appendLine(e.text, e.cls, true)
      );
    ...
    // ══════════════════════════════════════════════════════
      ...
      if (withoutSudo) {
        _appendLine(
          '> Stripping deprecated "sudo" prefix...',
          "cline-warn",
          true
        );
        _parse(withoutSudo);
    ...
    // Requires: RenderProfiler.js loaded before this file
    // ══════════════════════════════════════════════════════
    else if (base === "perf") {
      const prof = window.renderProfiler;
      if (!prof) {
        _appendLine(
          "ERR: RenderProfiler not loaded. Check index.html script order.",
          "cline-error"
        );
      } else {
        switch (sub) {
          case "start":
            prof.start();
            _appendLine("> RenderProfiler armed...", "cline-info");
            _appendLine(
              "✔ Profiler running. Use: perf snap <label>",
              "cline-ok"
            );
            break;
          case "stop":
            prof.stop();
            _appendLine("✔ Profiler stopped.", "cline-ok");
            break;
          case "reset":
            prof.reset();
            _appendLine(
              "✔ Profiler reset — all snapshots cleared.",
              "cline-ok"
            );
            break;
          case "snap": {
            const label =
              args.slice(2).join("_") || prof.getLiveStats().scenario;
            const snap = prof.snapshot(label);
            if (snap) {
              _appendLine(
                `> Saving snapshot [${snap.scenario}]...`,
                "cline-info"
              );
              _appendLine(
                `✔ avg=${snap.avgFPS} fps  |  1%low=${snap.p1low} fps  |  min=${snap.minFPS} fps  |  n=${snap.frames}`,
                "cline-ok"
              );
            } else {
              _appendLine(
                "ERR: Not enough frames (need ≥30). Is profiler running?",
                "cline-error"
              );
            }
            break;
          ...
          case "report":
            prof.report();
            _appendLine(
              "✔ Full report printed to browser console (F12 → Console).",
              "cline-ok"
            );
            break;
          case "stress":
            prof.stressParticles(1500);
            _appendLine(
              "> Emitting 1500 particles for stress test...",
              "cline-info"
            );
            _appendLine(
              "✔ Wait ~10s then: perf snap stress_particles",
              "cline-ok"
            );
            break;
          default:
            _appendLine(
              "Usage: perf [start | stop | reset | snap <label> | report | stress]",
              "cline-info"
            );
        }
      }
      ...
      if (input) {
        ...
        input._onKeydown = (e) => {
          ...
          if (e.key === "Tab") {
            ...
            if (matches.length === 0) {
              _appendLine(
                `> No completions for "${input.value}"`,
                "cline-warn",
                true
              );
              ...
              for (const m of matches) {
                let i = 0;
                while (i < prefix.length && i < m.length && prefix[i] === m[i])
                  i++;
...
function openAdminConsole() {
  ...
  if (!window.isAdmin) {
    ...
    console.log(
      "%c[MTC Admin] ⛔ Access denied — Developer Mode not active.",
      "color:#ef4444;"
    );
    return;
  }
  ...
  Audio.pauseBGM();
  console.log(
    "%c[MTC Admin] ✅ Admin Console opened.",
...
function drawDatabaseServer() {
  ...
  if (d < cfg.interactionRadius * 2.5) {
    ...
    if (d < cfg.interactionRadius * 1.2) {
      CTX.save();
      CTX.globalAlpha =
        Math.max(0, 1 - d / cfg.interactionRadius) * (0.7 + glow * 0.3);
      ...
      CTX.fillText(
        "[F] ACCESS DATABASE",
        screen.x,
        screen.y - cfg.interactionRadius - 12
      );
      CTX.restore();
    }
  }
