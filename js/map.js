...
let _mapNow = 0;
function _updateMapNow() {
  _mapNow = performance.now();
}
...
function drawTree(size) {
  ...
  CTX.beginPath();
  CTX.arc(0, -size * 0.78, size * 0.4, 0, Math.PI * 2);
  CTX.fill();
  ...
  for (let i = 0; i < 6; i++) {
    const a = (Math.PI / 3) * i - Math.PI / 6,
      r = size * 0.42;
    i === 0
      ? CTX.moveTo(Math.cos(a) * r, -size * 0.78 + Math.sin(a) * r)
      : CTX.lineTo(Math.cos(a) * r, -size * 0.78 + Math.sin(a) * r);
  }
  CTX.closePath();
  CTX.stroke();
}
...
function drawServer(w, h) {
  ...
  CTX.fillStyle = ds.headerFill;
  CTX.fillRect(4, 0, w - 8, 4);
  CTX.strokeStyle = ds.headerVent;
  CTX.lineWidth = 0.8;
...
function drawDataPillar(w, h) {
  ...
  grad.addColorStop(0, ddp.bodyGrad[0]);
  grad.addColorStop(0.4, ddp.bodyGrad[1]);
  grad.addColorStop(1, ddp.bodyGrad[2]);
  CTX.fillStyle = grad;
  CTX.fillRect(0, 0, w, h - 6);
  CTX.strokeStyle = `${ddp.circuit}${glow * 0.8})`;
  CTX.lineWidth = 1.2;
  CTX.shadowBlur = 6;
  ...
  CTX.shadowBlur = 0;
  CTX.fillStyle = `${ddp.circuit}${glow})`;
  CTX.shadowBlur = 10;
  CTX.shadowColor = pal.pillarCircuit;
  CTX.beginPath();
...
function drawBookshelf(w, h) {
  ...
  const shelfCount = 3,
    shelfH = (h - 6) / shelfCount;
  for (let s = 0; s < shelfCount; s++) {
    const sy = 3 + s * shelfH;
    CTX.fillStyle = db.shelfBoard;
    CTX.fillRect(5, sy + shelfH - 4, w - 10, 5);
    let bx = 7,
      bookIdx = (s * 7) % bookColors.length;
    while (bx < w - 12) {
      ...
      CTX.fill();
      CTX.fillStyle = db.bookGloss;
      CTX.fillRect(bx + 1, sy + shelfH - 4 - bh + 2, 2, bh - 4);
      CTX.fillStyle = db.bookShadow;
      CTX.fillRect(
        bx + bw * 0.25,
        sy + shelfH - 4 - bh + bh * 0.3,
        bw * 0.5,
        1.5
      );
      bx += bw + 1;
...
function drawDatabase(w, h) {
  ...
  CTX.beginPath();
  CTX.ellipse(w / 2, h + 8, w * 0.6, 10, 0, 0, Math.PI * 2);
  CTX.fill();
  ...
  CTX.beginPath();
  CTX.roundRect(0, 0, w, 14, [6, 6, 0, 0]);
  CTX.fill();
  ...
  const rackStartY = 18,
    unitH = Math.floor((h - rackStartY - 16) / 4);
  for (let u = 0; u < 4; u++) {
    const uy = rackStartY + u * unitH;
    CTX.fillStyle = pal.dbBody;
    CTX.beginPath();
    CTX.roundRect(6, uy, w - 12, unitH - 2, 2);
    CTX.fill();
    ...
    CTX.beginPath();
    CTX.arc(13, uy + unitH * 0.45, 3, 0, Math.PI * 2);
    CTX.fill();
    CTX.shadowBlur = 0;
    ...
    // Activity bar
    const actW =
      (w - 40) * (0.3 + Math.sin(now / (500 + u * 150) + u) * 0.3 + 0.1);
...
function drawCoopStore(w, h) {
  ...
  CTX.beginPath();
  CTX.roundRect(8, 12, w - 16, signH, 3);
  CTX.fill();
  ...
  const winY = 34,
    winH = Math.floor(h * 0.32);
  CTX.fillStyle = `rgba(249,115,22,${0.06 + pulse * 0.04})`;
  CTX.beginPath();
  CTX.roundRect(8, winY, w - 16, winH, 3);
  CTX.fill();
  ...
  for (let i = 0; i < itemCols; i++) {
    ...
    CTX.beginPath();
    CTX.roundRect(ix + 1, iy + 1, iw - 2, ih - 2, 2);
    CTX.fill();
    CTX.globalAlpha = 0.85;
  ...
  const counterH = Math.floor(h * 0.2);
  CTX.fillStyle = pal.shopCounter;
  CTX.beginPath();
  CTX.roundRect(6, counterY, w - 12, counterH, 3);
  CTX.fill();
  CTX.strokeStyle = `rgba(249,115,22,${0.3 + pulse * 0.15})`;
  CTX.lineWidth = 1;
  CTX.beginPath();
  ...
  CTX.beginPath();
  CTX.roundRect(w * 0.6, counterY + 4, w * 0.32, counterH - 8, 2);
  CTX.fill();
  ...
  CTX.stroke();
  // Register screen
  CTX.fillStyle = `rgba(249,115,22,${0.1 + pulse * 0.06})`;
  ...
  for (let s = 0; s < 5; s++) {
    ...
    CTX.beginPath();
    CTX.arc(sx, shelfY - 5, 4, 0, Math.PI * 2);
    CTX.fill();
    CTX.globalAlpha = 1;
  }
...
function drawVendingMachine(w, h) {
  ...
  CTX.fillStyle = "rgba(0,0,0,0.35)";
  CTX.beginPath();
  CTX.ellipse(w / 2, h + 5, w * 0.55, 7, 0, 0, Math.PI * 2);
  CTX.fill();
  ...
  CTX.beginPath();
  CTX.roundRect(5, 5, w - 10, Math.floor(h * 0.42), 3);
  CTX.fill();
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const ix = 8 + c * (itemW + 2),
        iy = 8 + r * (itemH + 2);
      CTX.fillStyle = "rgba(0,0,0,0.4)";
      CTX.beginPath();
      CTX.roundRect(ix, iy, itemW, itemH, 2);
      CTX.fill();
      CTX.fillStyle = itemColors[(r * cols + c) % itemColors.length];
      CTX.globalAlpha = 0.55 + pulse * 0.15;
      CTX.beginPath();
      CTX.roundRect(ix + 2, iy + 2, itemW - 4, itemH - 4, 1);
      CTX.fill();
      CTX.globalAlpha = 1;
    }
  }
  ...
  for (let b = 0; b < 3; b++) {
    ...
    CTX.beginPath();
    CTX.arc(7 + b * 11, btnY + 8, 4, 0, Math.PI * 2);
    CTX.fill();
    CTX.shadowBlur = 0;
  }
...
class MapObject {
  ...
  resolveCollision(entity) {
    ...
    if (distance < entity.radius) {
      ...
      // ── Push entity out ───────────────────────────────────────────
      let nx = 0,
        ny = 1; // surface normal (up by default when distance===0)
      if (distance > 0) {
        nx = dx / distance;
        ny = dy / distance;
        entity.x += nx * overlap;
      ...
      if (entity._aiMoveX !== undefined) {
        ...
        if (aDot < 0) {
          entity._aiMoveX -= aDot * nx;
          entity._aiMoveY -= aDot * ny;
        }
  ...
  draw() {
    ...
    switch (this.type) {
      case "desk":
        drawDesk(this.w, this.h);
        break;
      case "tree":
        drawTree(this.w / 2);
        break;
      case "server":
        drawServer(this.w, this.h);
        break;
      case "datapillar":
        drawDataPillar(this.w, this.h);
        break;
      case "bookshelf":
        drawBookshelf(this.w, this.h);
        break;
      case "blackboard":
        this.drawBlackboard();
        break;
      ...
      case "mtcwall":
        this.drawMTCWall();
        break; // New high-tech wall
      case "chair":
        this.drawChair();
        break; // กู้คืนป้องกัน Error
      case "cabinet":
        this.drawCabinet();
        break; // กู้คืนป้องกัน Error
      case "vendingmachine":
        drawVendingMachine(this.w, this.h);
        break;
      case "database":
        drawDatabase(this.w, this.h);
        break;
      case "coopstore":
        drawCoopStore(this.w, this.h);
        break;
    }
    CTX.restore();
  }
  ...
  drawMTCWall() {
    ...
    CTX.strokeStyle = `rgba(217,119,6,${0.25 + pulse * 0.4})`;
    CTX.lineWidth = 2;
    CTX.shadowBlur = 8;
  ...
  drawBlackboard() {
    ...
    CTX.beginPath();
    CTX.roundRect(5, 4, this.w - 10, this.h - 10, 2);
    CTX.fill();
    ...
    for (let i = 0; i < 3; i++) {
      ...
      CTX.beginPath();
      CTX.roundRect(this.w * 0.25 + i * 16, this.h - 7, 12, 4, 1);
      CTX.fill();
    }
    ...
    CTX.beginPath();
    CTX.ellipse(this.w * 0.5, this.h - 9, 40, 4, 0, 0, Math.PI * 2);
    CTX.fill();
  }
  ...
  drawWall() {
    ...
    for (let row = 0; row < Math.ceil(H / 20); row++) {
      ...
      CTX.beginPath();
      for (let bx = -25; bx < W + 25; bx += 50) {
        CTX.rect(bx + offset, ry, 50, 20);
      }
      CTX.stroke();
...
class ExplosiveBarrel extends MapObject {
  constructor(x, y) {
    ...
    this.radius = 28; // ลด 35→28 เดินผ่านได้ง่ายขึ้น
  }

  draw() {
    const screen = worldToScreen(this.x, this.y);
    const cx = screen.x + this.w / 2,
      cy = screen.y + this.h / 2;
    const now = _mapNow,
      W = this.w,
      H = this.h;
    ...
    const bodyR = Math.round(140 + (1 - hpFrac) * 80),
      bodyG = Math.round(20 + (1 - hpFrac) * 40);
    ...
    CTX.fillStyle = `rgb(${Math.round(bodyR * 0.7)},${Math.round(
      bodyG * 0.7
    )},15)`;
    CTX.beginPath();
    ...
    if (this.hp < this.maxHp) {
      ...
      CTX.fillRect(cx - 17, screen.y - 10, 34, 5);
      CTX.fillStyle = barCol;
      CTX.fillRect(cx - 17, screen.y - 10, 34 * pct, 5);
      CTX.strokeStyle = "rgba(0,0,0,0.80)";
      CTX.lineWidth = 1;
      CTX.strokeRect(cx - 17, screen.y - 10, 34, 5);
    }
  }
}
...
class PowerNode extends MapObject {
  ...
  draw() {
    ...
    // Pulsing core
    const pulse = (Math.sin(_mapNow / 200) + 1) / 2; // 0..1
    CTX.fillStyle = conf.activeColor;
...
class MTCRoom {
      ...
      if (!this.isPlayerInside) {
        ...
        if (window.UIManager)
          window.UIManager.showVoiceBubble(
            "เข้าสู่ MTC Room - เริ่มกระบวนการฟื้นฟู",
            player.x,
            player.y - 40
          );
  ...
  kickOut(player) {
    ...
    if (window.UIManager)
      window.UIManager.showVoiceBubble(
        "MTC Room เข้าสู่สถานะ Cooldown",
        player.x,
        player.y - 40
      );
  ...
  draw() {
    ...
    CTX.lineWidth = 2;
    CTX.moveTo(s.x + 6, s.y + 6 + bS);
    CTX.lineTo(s.x + 6, s.y + 6);
    CTX.lineTo(s.x + 6 + bS, s.y + 6);
    CTX.moveTo(s.x + W - 6 - bS, s.y + 6);
    CTX.lineTo(s.x + W - 6, s.y + 6);
    CTX.lineTo(s.x + W - 6, s.y + 6 + bS);
    CTX.moveTo(s.x + 6, s.y + H - 6 - bS);
    CTX.lineTo(s.x + 6, s.y + H - 6);
    CTX.lineTo(s.x + 6 + bS, s.y + H - 6);
    CTX.moveTo(s.x + W - 6 - bS, s.y + H - 6);
    CTX.lineTo(s.x + W - 6, s.y + H - 6);
    CTX.lineTo(s.x + W - 6, s.y + H - 6 - bS);
    CTX.stroke();
    ...
    // ── 2. CITADEL HEADER BAR (top banner) ───────────────────
    {
      ...
      CTX.beginPath();
      CTX.roundRect(s.x, s.y, W, barH, [4, 4, 0, 0]);
      CTX.fill();
    ...
    for (const rx of railPositions) {
      CTX.fillStyle = `rgba(250,180,30,${0.18 + fastPulse * 0.1})`;
      CTX.fillRect(rx, s.y + 22, 2, H - 44);
      // Single cap dot top + bottom
      CTX.fillStyle = `rgba(250,180,30,${0.55 + fastPulse * 0.35})`;
      CTX.beginPath();
      CTX.arc(rx + 1, s.y + 28, 3, 0, Math.PI * 2);
      CTX.fill();
      CTX.beginPath();
      CTX.arc(rx + 1, s.y + H - 28, 3, 0, Math.PI * 2);
      CTX.fill();
    }
    ...
    CTX.beginPath();
    CTX.roundRect(holoX, holoY, holoW, holoH, 8);
    CTX.fill();
    // Border
    CTX.strokeStyle = active
      ? `rgba(250,180,30,${0.55 + fastPulse * 0.3})`
      : "rgba(60,30,5,0.5)";
    CTX.lineWidth = 1.5;
    CTX.beginPath();
    CTX.roundRect(holoX, holoY, holoW, holoH, 8);
    ...
    if (active) {
      ...
      if (
        ...
        BALANCE.mtcRoom &&
        BALANCE.mtcRoom.buffCycleNames
      ) {
        const C = BALANCE.mtcRoom;
        ...
        CTX.fillStyle = "rgba(0,0,0,0.35)";
        CTX.beginPath();
        CTX.roundRect(holoX + 8, holoY + 3, holoW - 16, 11, 3);
        CTX.fill();
      ...
      if (pl && pl.mtcBuffTimer > 0 && pl.mtcBuffType >= 0) {
        ...
        CTX.fillText(
          `${buffName} ${pl.mtcBuffTimer.toFixed(1)}s`,
          cx,
          holoY + holoH + 11
        );
        CTX.globalAlpha = 1;
    ...
    if (active) {
      ...
      CTX.lineWidth = 7;
      CTX.beginPath();
      CTX.moveTo(s.x + 20, ffY);
      CTX.lineTo(s.x + W - 20, ffY);
      CTX.stroke();
      // Bright core
      ...
      for (const px of [s.x + 20, s.x + W - 20]) {
        ...
        CTX.beginPath();
        CTX.arc(px, ffY, 8, 0, Math.PI * 2);
        CTX.fill();
        CTX.fillStyle = `rgba(251,191,36,${0.85 + fastPulse * 0.15})`;
        CTX.beginPath();
        CTX.arc(px, ffY, 3, 0, Math.PI * 2);
        CTX.fill();
      }
      // SAFE ZONE label
      ...
      CTX.setLineDash([8, 6]);
      CTX.beginPath();
      CTX.moveTo(s.x + 16, ffY);
      CTX.lineTo(s.x + W - 16, ffY);
      CTX.stroke();
      CTX.setLineDash([]);
    ...
    if (this.isPlayerInside) {
      ...
      const timerColor =
        timerPct > 0.5 ? "#fbbf24" : timerPct > 0.25 ? "#f59e0b" : "#ef4444";
      const timerCx = cx,
        timerCy = s.y + H - 44;
      CTX.strokeStyle = "rgba(255,255,255,0.08)";
      CTX.lineWidth = 5;
      CTX.beginPath();
      CTX.arc(timerCx, timerCy, 18, 0, Math.PI * 2);
      CTX.stroke();
...
class MapSystem {
  constructor() {
    ...
    this._lightCtx = null;
    // Pattern cache — built once in _initPatternCanvases(), zero GC after that
    this._patternCache = null;
    this._patternCtxRef = null;
  }
  ...
  init() {
    ...
    const powerNodes = [
      ...
      { x: 450, y: 50 }, // East Center (Zone A approach)
      { x: -450, y: 50 }, // West Center (Zone B approach)
    ];

    for (const node of powerNodes) {
      if (!this._isClearZone(node.x, node.y, 40, 100)) {
        this.objects.push(new PowerNode(node.x, node.y));
      }
    }

    this.initialized = true;
    console.log(
      `✅ Campus Map Generated Structurally: ${this.objects.length} objects`
    );
  }
  ...
  _isClearZone(x, y, w = 0, h = 0) {
    const zones = [
      // Spawn area
      { type: "circle", x: 0, y: 0, r: 300 },
      // Citadel approach
      { type: "rect", x: -200, y: -500, w: 400, h: 180 },
      // Database approach
      { type: "rect", x: 360, y: -440, w: 280, h: 160 },
      // CoopStore approach
      { type: "rect", x: -640, y: 340, w: 260, h: 105 },
      // East gate gaps
      { type: "rect", x: 390, y: -230, w: 110, h: 140 },
      { type: "rect", x: 390, y: 95, w: 110, h: 70 },
      // West gate gaps
      { type: "rect", x: -510, y: -230, w: 130, h: 140 },
      { type: "rect", x: -510, y: 95, w: 130, h: 70 },
    ];
  ...
  generateCampusMap() {
    ...
    this.objects.push(
      new MapObject(mtcX - wallThick, mtcY, wallThick, mtcH, "mtcwall")
    );
    this.objects.push(
    ...
    for (const spot of barrelSpots) {
      ...
      for (const obj of this.objects) {
        if (Math.hypot(obj.x - spot.x, obj.y - spot.y) < 45) {
          tooClose = true;
          break;
        }
      }
  ...
  drawTerrain(ctx, camera) {
    ...
    // ── 1. ARENA BOUNDARY ────────────────────────────────────
    {
      ...
      ctx.globalAlpha = A.dashAlphaBase + Math.sin(t * 1.8) * 0.1;
    ...
    // ── 2. TECH-HEX GRID ─────────────────────────────────────
    {
      ...
      const viewL = camera.x - CANVAS.width * 0.5,
        viewT = camera.y - CANVAS.height * 0.5;
      const colStart = Math.floor(viewL / COL_STEP) - 1,
        colEnd =
          colStart + Math.ceil((CANVAS.width + HEX_W * 2) / COL_STEP) + 2;
      const rowStart = Math.floor(viewT / ROW_STEP) - 1,
        rowEnd =
          rowStart + Math.ceil((CANVAS.height + HEX_H * 2) / ROW_STEP) + 2;
      ...
      const _strokeRGB = H.strokeColor.substring(
        5,
        H.strokeColor.lastIndexOf(",")
      );
      ...
      for (let col = colStart; col <= colEnd; col++) {
        for (let row = rowStart; row <= rowEnd; row++) {
          const wx = col * COL_STEP,
            wy = row * ROW_STEP + (col % 2 === 0 ? 0 : HEX_H * 0.5);
          ...
          if ((Math.abs(col) + Math.abs(row)) % 3 === 0) {
            ctx.fillStyle = `rgba(${_fillRGB},${_fillBase * falloff})`;
            ctx.fill();
          }
          ctx.strokeStyle = `rgba(${_strokeRGB},${_strokeBase * falloff})`;
          ctx.stroke();
        }
      }
      ctx.restore();
    ...
    const drawCircuitPath = (pathCfg) => {
      ...
      const A = ws(pc.from.x, pc.from.y),
        B = ws(pc.to.x, pc.to.y),
        M = ws(pc.to.x, pc.from.y);
      ctx.save();
      ...
      ctx.globalAlpha =
        P.glowAlphaBase + 0.08 + Math.sin(t * 0.8 + pc.phase) * 0.06;
      ctx.beginPath();
      ctx.moveTo(A.x, A.y);
      ctx.lineTo(M.x, M.y);
      ctx.lineTo(B.x, B.y);
      ...
      const seg1Len = Math.hypot(M.x - A.x, M.y - A.y),
        seg2Len = Math.hypot(B.x - M.x, B.y - M.y),
        total = seg1Len + seg2Len;
      ...
      if (_drawPackets && total > 1) {
        for (let p = 0; p < P.packetCount; p++) {
          const progress =
            (((t * P.packetSpeed + pc.phase * 0.25 + p * (1 / P.packetCount)) %
              1) +
              1) %
            1;
      ...
      ctx.globalAlpha = 0.85 + Math.sin(t * 2.5 + pc.phase) * 0.1;
      ctx.fillStyle = pc.coreColor;
      ctx.shadowBlur = 12;
      ctx.shadowColor = pc.glowColor;
      ctx.beginPath();
    ...
    const drawZoneAura = (auraCfg) => {
      ...
      const sc = ws(ac.worldX, ac.worldY),
        edgePt = ws(ac.worldX + ac.radius, ac.worldY);
      const rSS = Math.abs(edgePt.x - sc.x),
        pulse = 0.5 + Math.sin(t * 1.5 + ac.phase) * 0.18;
      ...
      grad.addColorStop(
        0,
        `rgba(${ac.innerRgb}, ${(S.innerAlphaBase * pulse).toFixed(3)})`
      );
      grad.addColorStop(
        0.5,
        `rgba(${ac.innerRgb}, ${(S.midAlphaBase * pulse).toFixed(3)})`
      );
      grad.addColorStop(
        0.8,
        `rgba(${ac.outerRgb}, ${(S.outerAlphaBase * pulse).toFixed(3)})`
      );
    ...
    if (C.landmark) {
      ...
      const aOuter = LM.outerAlphaBase + pulse * 0.3;
      ...
      ctx.beginPath();
      ctx.arc(ox, oy, rInner, 0, Math.PI * 2);
      ctx.stroke();
      ...
      for (let i = 0; i < LM.spokeCount; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(
          ox + Math.cos(angle) * rInner,
          oy + Math.sin(angle) * rInner
        );
        ctx.lineTo(
          ox + Math.cos(angle) * rOuter,
          oy + Math.sin(angle) * rOuter
        );
        ctx.stroke();
      }
  ...
  // ── Pattern canvas factory — runs ONCE per canvas context ──────────────
  // Builds offscreen tile canvases and calls ctx.createPattern() once.
  // After first call: this._patternCache[key] === CanvasPattern, no GC.
  _initPatternCanvases(ctx) {
    if (this._patternCache && this._patternCtxRef === ctx) return;
    this._patternCtxRef = ctx;
    this._patternCache = {};
    ...
    const mk = (pw, ph, fn) => {
      ...
      oc.width = pw;
      oc.height = ph;
      fn(oc.getContext("2d"), pw, ph);
      return ctx.createPattern(oc, "repeat");
    };

    // serverFarm — horizontal CRT scanlines (cyan, 2×8 px)
    this._patternCache.serverFarm = mk(2, 8, (c, w) => {
      c.fillStyle = "rgba(6,182,212,0.18)";
      c.fillRect(0, 0, w, 1);
    });

    // library — topographic diagonal hatch 45° (amber, 20×20 px)
    // Single line (0,20)→(20,0) tiles seamlessly into parallel 45° hatches.
    this._patternCache.library = mk(20, 20, (c, w, h) => {
    ...
    // courtyard — targeting reticle dot stipple (green, 24×24 px)
    this._patternCache.courtyard = mk(24, 24, (c, w, h) => {
      c.fillStyle = "rgba(74,222,128,0.28)";
      c.beginPath();
      c.arc(w / 2, h / 2, 1.5, 0, Math.PI * 2);
      c.fill();
    });

    // lectureHalls — blueprint cross-hatch (purple, 16×16 px) — shared ref
    const lecPat = mk(16, 16, (c, w, h) => {
    ...
    this._patternCache.lectureHallL = lecPat;
    this._patternCache.lectureHallR = lecPat;
    // database + shop: no floor pattern (landmarks use aura system)
  }

  drawZoneFloors(ctx) {
    if (typeof MAP_CONFIG === "undefined" || !MAP_CONFIG.zones) return;
    this._initPatternCanvases(ctx); // no-op after first call

    const t = _mapNow * 0.001;
    const zoneKeys = Object.keys(MAP_CONFIG.zones);

    // Static zone metadata — bracket color + tile dimensions, no per-frame alloc.
    // bc  = bracket/tag solid hex color (derived from accentColor)
    // pw/ph = pattern tile width/height in px (0 = no pattern for this zone)
    const ZONE_META = {
      serverFarm: { id: "SRV-A", bc: "#22d3ee", pw: 2, ph: 8 },
      library: { id: "LIB-B", bc: "#fde047", pw: 20, ph: 20 },
      courtyard: { id: "CYD-C", bc: "#86efac", pw: 24, ph: 24 },
      lectureHallL: { id: "LEC-D1", bc: "#d8b4fe", pw: 16, ph: 16 },
      lectureHallR: { id: "LEC-D2", bc: "#d8b4fe", pw: 16, ph: 16 },
      database: { id: "DB-NE", bc: "#fbbf24", pw: 0, ph: 0 },
      shop: { id: "SHP-SW", bc: "#fb923c", pw: 0, ph: 0 },
    };

    // World origin in screen space — used for pattern world-alignment.
    // Computed once per drawZoneFloors call (not per zone).
    const originSS = worldToScreen(0, 0);

    for (let zi = 0; zi < zoneKeys.length; zi++) {
      const key = zoneKeys[zi];
      const z = MAP_CONFIG.zones[key];
      const meta = ZONE_META[key] || { id: key, bc: "#ffffff", pw: 0, ph: 0 };
      ...
      // Viewport cull
      if (br.x < 0 || tl.x > CANVAS.width || br.y < 0 || tl.y > CANVAS.height)
        continue;

      const pulse = 0.5 + Math.sin(t * 1.2 + zi) * 0.5;
      // Bracket arm length — clamped so brackets don't collide on tiny zones
      const bL = Math.min(28, sw * 0.18, sh * 0.18);

      ctx.save();
      ctx.beginPath();
      ctx.rect(tl.x, tl.y, sw, sh);
      ctx.clip(); // all subsequent draws are clipped to this zone
      ...
      // ── 2. Tactical terrain pattern (Direction A) ─────────────────────
      // Pattern phase is derived from world origin in screen space so the
      // tile stays locked to world coordinates as the camera pans.
      const pat = this._patternCache[key];
      if (pat && meta.pw > 0) {
        const pw = meta.pw,
          ph = meta.ph;
        const phX = ((originSS.x % pw) + pw) % pw;
        const phY = ((originSS.y % ph) + ph) % ph;
        ctx.save();
        // Translate so pattern corner aligns with world origin → world-locked.
        ctx.translate(phX, phY);
        ctx.fillStyle = pat;
        // Expand rect by one tile on each side to guarantee full zone coverage.
        ctx.fillRect(
          tl.x - phX - pw,
          tl.y - phY - ph,
          sw + pw * 2,
      ...
      // ── 3. Radial vignette edge fade (Direction B) ────────────────────
      // One createRadialGradient per large zone per frame — acceptable cost.
      // Small landmark zones (database, shop) skip vignette; they have auras.
      if (sw > 150 && sh > 150) {
        const cgx = tl.x + sw * 0.5,
          cgy = tl.y + sh * 0.5;
        const r = Math.max(sw, sh) * 0.72;
        const vgr = ctx.createRadialGradient(cgx, cgy, r * 0.4, cgx, cgy, r);
        vgr.addColorStop(0, "rgba(0,0,0,0)");
        vgr.addColorStop(1, "rgba(0,0,0,0.50)");
        ctx.globalAlpha = 1;
        ctx.fillStyle = vgr;
        ctx.fillRect(tl.x, tl.y, sw, sh);
      }

      // ── 4. HUD corner brackets (Direction C) ──────────────────────────
      ctx.globalAlpha = 0.62 + pulse * 0.28;
      ctx.strokeStyle = meta.bc;
      ctx.lineWidth = 1.5;
      ctx.lineCap = "square";

      // TL ⌐
      ctx.beginPath();
      ctx.moveTo(tl.x + 4, tl.y + 4 + bL);
      ctx.lineTo(tl.x + 4, tl.y + 4);
      ctx.lineTo(tl.x + 4 + bL, tl.y + 4);
      ctx.stroke();
      // TR ¬
      ctx.beginPath();
      ctx.moveTo(br.x - 4 - bL, tl.y + 4);
      ctx.lineTo(br.x - 4, tl.y + 4);
      ctx.lineTo(br.x - 4, tl.y + 4 + bL);
      ctx.stroke();
      // BL L
      ctx.beginPath();
      ctx.moveTo(tl.x + 4, br.y - 4 - bL);
      ctx.lineTo(tl.x + 4, br.y - 4);
      ctx.lineTo(tl.x + 4 + bL, br.y - 4);
      ctx.stroke();
      // BR ⌐ (mirrored)
      ctx.beginPath();
      ctx.moveTo(br.x - 4 - bL, br.y - 4);
      ctx.lineTo(br.x - 4, br.y - 4);
      ctx.lineTo(br.x - 4, br.y - 4 - bL);
      ctx.stroke();
      ...
      if (sh > bL + 30) {
        ...
        const bEndY = tl.y + 4 + bL; // bottom of TR bracket arm
        const tagTop = bEndY + 3;
        const tagRight = br.x - 5;
        const tagLeft = tagRight - tw - 14;
        const tagH = 13;
        const blinkOn = Math.sin(t * 2.8 + zi) > 0;

        // Connector line: bracket-end → tag top
        ctx.globalAlpha = 0.36 + pulse * 0.16;
        ctx.strokeStyle = meta.bc;
        ctx.lineWidth = 0.8;
        ctx.beginPath();
        ctx.moveTo(br.x - 4, bEndY);
        ctx.lineTo(br.x - 4, tagTop);
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.roundRect(tagLeft, tagTop, tw + 14, tagH, 2);
        ctx.fill();
        ...
        // Blink dot — sin > 0 toggles every ~1.1s (2.8 rad/s period)
        if (blinkOn) {
          ctx.globalAlpha = 0.85;
          ctx.fillStyle = meta.bc;
          ctx.beginPath();
          ctx.arc(tagLeft + 5, tagTop + tagH * 0.5, 1.8, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        ctx.fillStyle = meta.bc;
        ctx.fillText(meta.id, tagRight - 3, tagTop + 1);
        ctx.globalAlpha = 1;
      }

      // ── 6. Coordinate readout (bottom-left, very faint) ───────────────
      {
        const cx = Math.round(z.x + z.w * 0.5);
        const cy = Math.round(z.y + z.h * 0.5);
        const s = (v) => (v >= 0 ? "+" : "") + v;
      ...
      // ── 7. Ambient micro-particles (unchanged — deterministic) ─────────
      // PHI-spaced seeds prevent all dots pulsing in sync.
      // Outer clip already constrains particles to zone — no inner clip needed.
      if (z.ambientColor) {
        const PHI = 2.399;
        const COUNT = 7;
        for (let i = 0; i < COUNT; i++) {
          ...
          const wx =
            z.x +
            z.w * (0.15 + 0.7 * ((Math.sin(seed * 3.7) + 1) * 0.5)) +
            Math.sin(t * 0.4 + seed) * 18;
          const wy =
            z.y +
            z.h * (0.15 + 0.7 * ((Math.cos(seed * 2.9) + 1) * 0.5)) +
            Math.cos(t * 0.5 + seed * 1.3) * 14;
          const ss = worldToScreen(wx, wy);
          const alpha = 0.25 + Math.sin(t * 1.1 + seed) * 0.2;
  ...
  drawLighting(player, projectiles = [], extraLights = []) {
    ...
    // ── Phase 2 Red Overlay Logic ──────────────────────
    let nightR = L.nightR,
      nightG = L.nightG,
      nightB = L.nightB,
      nightA = darkness;
    const manop = window.enemies
      ? window.enemies.find((e) => e instanceof KruManop)
      : null;
    if (manop && manop.phase >= 2 && !manop.dead) {
      nightR = 139;
      nightG = 0;
      nightB = 0; // Dark Red
      nightA = Math.max(0.45, darkness); // Force at least 0.45 opacity
    }
    lctx.fillStyle = `rgba(${nightR},${nightG},${nightB},${nightA.toFixed(3)})`;
    ...
    const punchLight = (wx, wy, radius, type = "neutral", intensity = 1.0) => {
      ...
      if (type === "warm") {
        ...
        tInner = `rgba(74,222,128,${tAlpha})`;
        tOuter = "rgba(34,197,94,0)";
      } else {
    ...
    if (player && !player.dead)
      punchLight(
        ...
        L.playerLightRadius,
        "warm",
        player.isDashing ? 1.25 : 1.0
      );

    if (
      typeof window.projectiles !== "undefined" &&
      Array.isArray(window.projectiles)
    ) {
      for (const proj of window.projectiles) {
        ...
        punchLight(
          proj.x,
          proj.y,
          L.projectileLightRadius,
          proj.team === "player" ? "cool" : "warm"
        );
      }
    }

    const MAX_LIGHT_RADIUS =
      Math.max(L.dataPillarLightRadius, L.serverRackLightRadius) + 40;
    for (const obj of this.objects) {
      if (obj.type === "datapillar" || obj.type === "server") {
        const cx = obj.x + obj.w * 0.5,
          cy = obj.type === "datapillar" ? obj.y - 5 : obj.y + obj.h * 0.4,
          sp = toSS(cx, cy);
        if (sp.x + MAX_LIGHT_RADIUS < 0 || sp.x - MAX_LIGHT_RADIUS > lc.width)
          continue;
        if (sp.y + MAX_LIGHT_RADIUS < 0 || sp.y - MAX_LIGHT_RADIUS > lc.height)
          continue;
      }
      if (obj.type === "datapillar")
        ...
        punchLight(obj.x + obj.w * 0.5, obj.y + obj.h * 0.2, 45, "green");
      else if (obj.type === "database")
        punchLight(
          obj.x + obj.w * 0.5,
          obj.y + obj.h * 0.3,
          L.mtcServerLightRadius,
          "warm"
        );
      else if (obj.type === "coopstore")
        punchLight(
          obj.x + obj.w * 0.5,
          obj.y + obj.h * 0.3,
          L.shopLightRadius,
    ...
    if (this.mtcRoom) {
      const rx = this.mtcRoom.x + this.mtcRoom.w * 0.5,
        ry = this.mtcRoom.y + this.mtcRoom.h * 0.5;
        ...
        if (typeof spawnParticles === "function") {
          const dustColor =
            obj.type === "wall"
              ...
              ? "#4ade80"
              : "#e2e8f0";
          spawnParticles(cx, cy, 20, dustColor);
          spawnParticles(cx, cy, 8, "#ff4500");
        }
        if (typeof spawnFloatingText === "function")
          spawnFloatingText("💥 DESTROYED", cx, cy - 24, "#f97316", 20);
    ...
    if (this.mtcRoom) {
      ...
      if (lineHitsAABB(startX, startY, endX, endY, r.x, r.y, r.w, r.h)) {
        const rcx = r.x + r.w * 0.5,
          rcy = r.y + r.h * 0.5;
        if (typeof spawnParticles === "function") {
          spawnParticles(rcx, rcy, 15, "#38bdf8");
          spawnParticles(rcx, rcy, 5, "#ff4500");
        }
        if (typeof spawnFloatingText === "function")
          spawnFloatingText("🛡️ SHIELD HIT!", rcx, rcy - 35, "#38bdf8", 22);
      }
    }
  }
  ...
  findSafeSpawn(preferredX, preferredY, radius) {
    for (let i = 0; i < 50; i++) {
      ...
      const x = preferredX + Math.cos(angle) * distance,
        y = preferredY + Math.sin(angle) * distance;
